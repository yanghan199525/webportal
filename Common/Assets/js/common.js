...
showForm({
        ...
        url: path + "/Portal/Ultimus.UWF.OrgChart/SelectOrg.aspx?Type=" + type + "&data=" + olddata,
        returnFunc: "getData"
});
*/
function showForm(options) {
    ...
    if (!buttons) {
        buttons = [{
            ...
            label: canceltext,
            cssClass: 'btn btn-close btn-md',
            action: function (dialog) {
                if (typeof (BootstrapDialogClose) == "function") {
                    BootstrapDialogClose();
                }
                dialog.close();
            }
        }];
    ...
    BootstrapDialog.show({
        ...
        closeByBackdrop: bialogCloseByBackdrop,
...
function addRow(tabId) {
    try {
        ...
        if ($("#" + tabId + "_Tr_Count").length > 0) {
            modelTr = tabCtl.rows[tabCtl.rows.length - 2];
            rowIndex = tabCtl.rows.length - 2;
        }
...
objReport.openForm = function (formID, ProcessName, Incident) {
    ...
    var swidth = screen.width - 10;
    var winoption = "left=0,top=0,height=" + sheight + ",width=" + swidth + ",toolbar=yes,menubar=yes,location=yes,status=yes,scrollbars=yes,resizable=yes";
    s = window.open('/Portal/Ultimus.UWF.Home.V3/Workflow/OpenForm.aspx?FormID=' + formID + '&ProcessName=' + ProcessName + '&Incident=' + Incident + '&Type=report', '', winoption);
...
function validateCheckbox(field, rules, i, options) {
    ...
    var groupSize = $(field).find("input[data-field='" + groupname + "']:checked").size();
    if (groupSize < 1) {
        rules.push('required');
    }
}
...
function InitDateControls() {
    if (request("type").toLocaleLowerCase() != "myrequest") {
        ...
        try {
            ...
            $('input[data-type="date"]').attr({ onkeydown: "return false;", onclick: "$(this).blur()", placeholder: "YYYY/MM/DD" });
            $('input[data-type="datetime"]').attr({ onkeydown: "return false;", onclick: "$(this).blur()", placeholder: "YYYY/MM/DD  HH:mm" });
            $('input[data-type="time"]').attr({ onkeydown: "return false;", onclick: "$(this).blur()", placeholder: "HH:mm" });
        } catch (e) {
            $('input[data-type="date"]').attr({ onkeydown: "return false;", onclick: "$(this).blur()", placeholder: "YYYY/MM/DD" });
            $('input[data-type="datetime"]').attr({ onkeydown: "return false;", onclick: "$(this).blur()", placeholder: "YYYY/MM/DD  HH:mm" });
            $('input[data-type="time"]').attr({ placeholder: "HH:mm" });
        }
    }
}
