<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MyRequestList.aspx.cs" Inherits="Ultimus.UWF.Home.V3.MyRequestList" %>
...
<head>
    ...
    <title><%=Lang.Get("title_MyRequestList") %></title>
...
<body>
            ...
            <div class="page-header">
                ...
                <div class="right">
                    <div class="btn-group" role="group" aria-label="...">
                        ...
                        <%--    <a class="btn btn-light" onclick="$('#div_taskList').toggle()"><i class="fa fa-minus"></i></a>
                        <a class="btn btn-light" onclick="$('#li_expand').click()"><i class="fa fa-expand"></i></a>--%>
                    </div>
                </div>
                                        ...
                                        <div class="col-md-4 col-sm-6 col-xs-12">
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-7  col-xs-7">
                                                    <asp:DropDownList ID="ddlProcessCategory" CssClass="form-control" runat="server" AutoPostBack="true" OnSelectedIndexChanged="ddlProcessCategory_SelectedIndexChanged"></asp:DropDownList>
                        ...
                        <div class="panel panel-default">
                            ...
                            <div class="panel-body">
                                <table id="tasklist" class="table table-hover table-nofooter" width="100%" style="display: none">
                                    <thead>
                                        <tr>
                                            <td class="text-center hidden-xs hidden" data-orderable="false" hidden="hidden">
                                            ...
                                            <td class="hidden" style="min-width: 50px"><%=Lang.Get("TaskList_Incident") %></td>
                                            <td style="min-width: 140px"><%=Lang.Get("TaskList_ProcessName") %></td>
                                            <td><%=Lang.Get("TaskList_Summary") %></td>
                                            <%--<td class="hidden-xs" style="min-width: 50px"><%=Lang.Get("TaskList_StepName") %></td>--%>
                                    ...
                                    <tbody class="cursor-pointer">
                                        <asp:Repeater ID="rptTask" runat="server">
                                            <ItemTemplate>
                                                <tr>
                                                    <td class="text-center hidden-xs hidden" hidden="">
                                                    ...
                                                    <td class="hidden" onclick="javascript:openForm('<%#Eval("TaskID") %>','<%=Request.QueryString["Type"] %>','<%#Eval("ServerName") %>','<%#Eval("ProcessName") %>','<%#Eval("StepLabel") %>',this,'<%#Eval("PROCESSSTATUS").ToString() %>');">
                                                        <%#Eval("INCIDENT")%></td>
                                                    ...
                                                    <%--<td class="hidden-xs" onclick="javascript:openForm('<%#Eval("TaskID") %>','<%=Request.QueryString["Type"] %>','<%#Eval("ServerName") %>','<%#Eval("ProcessName") %>','<%#Eval("StepLabel") %>',this,'<%#Eval("PROCESSSTATUS").ToString() %>');">
                                                        <%#Eval("STEPLABEL")%></td>--%>
                                                    <td class="hidden-xs utcdatetime" onclick="javascript:openForm('<%#Eval("TaskID") %>','<%=Request.QueryString["Type"] %>','<%#Eval("ServerName") %>','<%#Eval("ProcessName") %>','<%#Eval("StepLabel") %>',this,'<%#Eval("PROCESSSTATUS").ToString() %>');">
                                                    ...
                                                    <td class="hidden-xs" onclick="javascript:openForm('<%#Eval("TaskID") %>','<%=Request.QueryString["Type"] %>','<%#Eval("ServerName") %>','<%#Eval("ProcessName") %>','<%#Eval("StepLabel") %>',this,'<%#Eval("PROCESSSTATUS").ToString() %>');">
                                                        <span class="<%#GetStatusClass(Eval("PROCESSSTATUS").ToString()) %>"><%#GetStatus(Eval("PROCESSSTATUS").ToString()) %></span></td>
                                                </tr>
                                            </ItemTemplate>
                                        </asp:Repeater>
                                    </tbody>
    ...
    <script type="text/javascript">
        ...
        //    let url = 'Workflow/OpenForm.aspx?TaskId=' + taskId + '&Type=MYREQUEST&ServerName=' + serverName + '&processStatus=' + processStatus + '&ProcessName=' + encodeURI(processName) + '&StepName=' + encodeURI(stepName);
        //    location.href = url;
        ...
        function openForm(taskId, type, serverName, processName, stepName, ele, processStatus) {
            ...
            s = window.open('Workflow/OpenForm.aspx?TaskId=' + taskId + '&Type=MYREQUEST&ServerName=' + serverName + '&processStatus=' + processStatus + '&ProcessName=' + encodeURI(processName) + '&StepName=' + encodeURI(stepName), '', winoption);
            s.focus();
            if ($('#txtType').val() == 'mytask') {
                try {
                    ele.parentNode.parentNode.style.display = 'none';
