<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="MyUnread.aspx.cs" Inherits="Ultimus.UWF.Home.V3.MyUnread" %>
...
<body>
    <form id="form1" runat="server">
        <div class="form-content">
            ...
            <div class="page-header">
                ...
                <h1 class="title ">
                    <span class="btn btn-rounded btn-default btn-icon cursor-default"><i class="fa fa-eye-slash"></i></span>
                    <%=Lang.Get("MyUnread")%></h1>
                <%-- <ol class="breadcrumb">
                                        ...
                                        <asp:Repeater ID="rptTask" runat="server">
                                            <ItemTemplate>
                                                <tr>
                                                    ...
                                                    <td onclick="javascript:openForm('<%#Eval("TaskID") %>','<%=Request.QueryString["Type"] %>','<%#Eval("ServerName") %>',this);">
                                                        <span class="" title='<%#Eval("SUMMARY")%>'><%#Eval("SUMMARY").ToString().Trim().Length>68?Eval("SUMMARY").ToString().Trim().Substring(0,67)+"...":Eval("SUMMARY").ToString().Trim()%></span>
                                                    </td>
                                                    ...
                                                    <td class="hidden-xs" onclick="javascript:openForm('<%#Eval("TaskID")%>','<%=Request.QueryString["Type"]%>','<%#Eval("ServerName")%>',this);">
                                                        <%# getIncidentsStatus(Eval("PROCESSNAME").ToString(),Eval("INCIDENT").ToString()) %>
                                                    </td>
                                                    <td class="hidden-xs" onclick="javascript:openForm('<%#Eval("TaskID")%>','<%=Request.QueryString["Type"]%>','<%#Eval("ServerName")%>',this);">
    ...
    <script type="text/javascript">
        function openForm(taskId, type, serverName, ele) {
            ...
            s = window.open('Workflow/OpenForm.aspx?TaskId=' + taskId + '&Type=MYUNREAD&ServerName=' + serverName, '', winoption);
