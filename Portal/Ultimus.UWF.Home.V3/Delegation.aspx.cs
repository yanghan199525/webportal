...
namespace Ultimus.UWF.Home.V3
{
    ...
    {  
        ...
        private void BingProcess()
        {
             ...
             dropProcessName.Items.Insert(0, new ListItem("所有", "ALL"));
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            if (pMode == 1)
            {
                ...
                foreach (string pTaskID in TaskIDArray)
                {
                    if (pTaskID.Trim() == "")
                        continue;
                    result = this.SetAssign(pTaskID, pFromUser, pToUser, pMode, DateTime.Parse(txtStartDate.Text), DateTime.Parse(pEndDate));
                }
            }
            else if (pMode == 3)
            {
            ...
            else if (pMode == 4)
            {
                ...
                string dBegin = this.txtStartDate.Text;
                string dEnd = this.txtEndDate.Text + " 23:59:59";
                //if (this.isExistAssign(strProcessName, pFromUser, dBegin))
                //{
                //    MessageBox("alert('" + Lang.Get("SubmitSuccess") + "')");
                //    return;
                //}
                if (strProcessName == "All")
                {
                    strProcessName = "";
                }
                result = this.SetProcAssign(strProcessName, pFromUser, pToUser, dBegin, dEnd);
            ...
            {
                //将来指派和所有任务指派不需要循环
                result = this.SetAssign("", pFromUser, pToUser, pMode, DateTime.Parse(txtStartDate.Text), DateTime.Parse(pEndDate));
            }
            if (result)
            {
