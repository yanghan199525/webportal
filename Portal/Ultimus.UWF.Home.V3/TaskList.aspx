<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TaskList.aspx.cs" Inherits="Ultimus.UWF.Home.V3.TaskList" %>
...
<head>
    ...
    <title><%=Lang.Get("ProcessSearch") %></title>
    <!-- ========== Css Files ========== -->
                            ...
                            <div class="panel-body">
                                ...
                                <div class="col-md-8 col-sm-6 col-xs-12">
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-7  col-xs-7" style="left: -200px">
                                ...
                                <table id="tasklist" class="table table-hover table-nofooter" width="100%">
                                    <thead>
                                        <tr>
                                            ...
                                            <td class="hidden-xs" style="min-width: 60px"><%=Lang.Get("TaskList_StepName") %></td>
                                            <td class="hidden-xs" style="min-width: 120px"><%=Lang.Get("TaskList_StartTime")%></td>
                                            <td class="hidden-xs" style="min-width: 60px"><%=Lang.Get("TaskList_Status")%></td>
                                            <td class="hidden-xs" style="min-width: 60px"><%=Lang.Get("TaskList_Applicant")%></td>
                                        </tr>
                                    </thead>
                                        ...
                                        <asp:Repeater ID="rptTask" runat="server">
                                            <ItemTemplate>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <span class="btn btn-icon btn-sm " onclick="window.open('TaskStatus.aspx?ProcessName=<%#Server.UrlEncode(Eval("PROCESSNAME").ToString().Trim()) %>&Incident=<%#Eval("INCIDENT") %>&TaskId=<%#Eval("TaskId") %>&ServerName=<%#Server.UrlEncode(MyLib.ConvertUtil.ToString(Eval("ServerName")).Trim()) %>&t=<%=Guid.NewGuid().ToString() %>');">
                                                            <i class="fa fa-line-chart"></i></span>
                                                    </td>
                                                    <td class="hidden" onclick="javascript:openForm('<%#Eval("TaskID") %>','<%=Request.QueryString["Type"] %>','<%#Eval("ServerName") %>','<%#Eval("ProcessName") %>','<%#Eval("StepLabel") %>',this);">
                                                        <%#Eval("INCIDENT")%></td>
                                                    <td onclick="javascript:openForm('<%#Eval("TaskID") %>','<%=Request.QueryString["Type"] %>','<%#Eval("ServerName") %>','<%#Eval("ProcessName") %>','<%#Eval("StepLabel") %>',this);">
                                                        <%#Lang.Get(MyLib.ConvertUtil.ToString(Eval("PROCESSNAME")))%></td>
                                                    <td onclick="javascript:openForm('<%#Eval("TaskID") %>','<%=Request.QueryString["Type"] %>','<%#Eval("ServerName") %>','<%#Eval("ProcessName") %>','<%#Eval("StepLabel") %>',this);">

                                                        <span class="color2"><%#Eval("DisplaySummary")%></span></td>
                                                    ...
                                                    <td class="hidden-xs" onclick="javascript:openForm('<%#Eval("TaskID") %>','<%=Request.QueryString["Type"] %>','<%#Eval("ServerName") %>','<%#Eval("ProcessName") %>','<%#Eval("StepLabel") %>',this);">
                                                        <span class="label label-default"><%#GetStatus(Eval("PROCESSSTATUS").ToString()) %></span></td>
                                                    <td class="hidden-xs" onclick="javascript:openForm('<%#Eval("TaskID")%>','<%=Request.QueryString["Type"]%>','<%#Eval("ServerName")%>','<%#Eval("ProcessName") %>','<%#Eval("StepLabel") %>',this);">
                                                        <span><%#GetUserCN(MyLib.ConvertUtil.ToString(Eval("INITIATOR")))%></span></td>
                                                </tr>
                                            </ItemTemplate>
                                        </asp:Repeater>
                                    </tbody>
