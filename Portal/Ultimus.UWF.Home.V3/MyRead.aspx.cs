...
namespace Ultimus.UWF.Home.V3
{
    public partial class MyRead : System.Web.UI.Page
    {
        ...
        private List<WF_READS> getReadsList(ref List<WF_READS> readsList)
        {
            ...
            {
                readsList = readsList.FindAll(p => Lang.Get(MyLib.ConvertUtil.ToString(p.PROCESSNAME)).ToUpper().Contains(txtProcessName.Text.Trim().ToUpper()));
            }
            ...
            {
                readsList = readsList.FindAll(p => p.INCIDENT == ConvertUtil.ToInt32(txtIncident.Text.Trim()));
        ...
        void BindGrid()
        {
            ...
            string sql = "SELECT  max(ID) ID, '' ServerName, TASKID, PROCESSNAME,SUMMARY, INCIDENT, STEPID, APPLICANT,APPLICANTNAME, STATUS, READFLAG, READER, STEPLABEL, STARTTIME FROM WF_READS WHERE READFLAG = 1 and STATUS=0 AND READER = '" + loginUser.LOGINNAME.Replace("CustomOC\\", "CustomOC/").ToString() + "'GROUP BY TASKID, PROCESSNAME, INCIDENT, STEPID, APPLICANT,APPLICANTNAME, STATUS, READFLAG, READER, STEPLABEL, STARTTIME, SUMMARY ORDER BY ID DESC"; //READFLAG=1 => 已阅
        ...
        public string GetUserCN(string APPLICANT)
        {
            ...
            try
            {
                ...
                string sql = @"select CNNAME from v_ORG_USER where ACCOUNT=N'" + APPLICANT.Replace("'", "''") + "'";
