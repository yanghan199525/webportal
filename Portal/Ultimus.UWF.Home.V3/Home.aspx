<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Home.aspx.cs" Inherits="Ultimus.UWF.Home.V3.Home" %>
...
<body>
    <form id="form1" runat="server">
            <iframe id="frmLink" src=""  width="100%" height="<%=HEIGTH%>" scrolling="no" frameborder="0"></iframe>
                ...
                <div class="right">
                    <div class="btn-group" role="group" aria-label="...">
                        ...
                        <ul class="dropdown-menu" role="menu">
                            <li><a href="javascript:assign();"><i class="fa fa-share padding-r-5"></i><%=Lang.Get("TaskList_Assign")%></a></li>
                            <li><a href="AssignmentList.aspx"><i class="fa fa-reply padding-r-5"></i><%=Lang.Get("TaskList_AssignCallback")%></a></li>
                            <li class="divider"></li>
                    ...
                    <div class="col-md-12">
                        <div class="panel panel-default">
                            ...
                            <div class="panel-title">
                                ...
                                <%=Lang.Get("DEFAULT_MYTASK")%><%=Lang.Get("ListInfo")%>
                                <ul class="panel-tools">
                                    <li><a class="icon " href="javascript:location.href=location.href;"><i class="fa fa-refresh"></i></a></li>
                                    <li><a class="icon minimise-tool"><i class="fa fa-minus"></i></a></li>
                                    <li><a class="icon expand-tool"><i class="fa fa-expand"></i></a></li>
                                </ul>
                            </div>
                            ...
                            <div class="panel-body">
                                <table id="tasklist" class="table table-hover table-nofooter" width="100%" style="display:none">
                                    <thead>
                                        <tr>
                                            <td class="text-center hidden-xs" data-orderable="false"></td>
                                            ...
                                            <td style="min-width:50px"><%=Lang.Get("TaskList_ProcessName") %></td>
                                            <td><%=Lang.Get("TaskList_Summary") %></td>
                                            <td class="hidden-xs" style="min-width:50px"><%=Lang.Get("TaskList_StepName") %></td>
                                            <td class="hidden-xs"><%=Lang.Get("TaskList_StartTime")%></td>
                                            <td class="hidden-xs"><%=Lang.Get("TaskList_Status")%></td>
                                        </tr>
                                    </thead>
                                        ...
                                        <asp:Repeater ID="rptTask" runat="server">
                                            <ItemTemplate>
                                                <tr>
                                                    ...
                                                    <td onclick="javascript:openForm('<%#Eval("TaskID")%>','<%=Request.QueryString["Type"]%>','<%#Eval("ServerName")%>','<%#Eval("ProcessName") %>','<%#Eval("StepLabel") %>',this);">
                                                        <span class=""><%#Eval("DisplaySummary")%></span></td>
                                                    <td class="hidden-xs" onclick="javascript:openForm('<%#Eval("TaskID")%>','<%=Request.QueryString["Type"]%>','<%#Eval("ServerName")%>','<%#Eval("ProcessName") %>','<%#Eval("StepLabel") %>',this);">
                                                        <%#Eval("STEPLABEL")%></td>
                                                    <td class="hidden-xs" onclick="javascript:openForm('<%#Eval("TaskID")%>','<%=Request.QueryString["Type"]%>','<%#Eval("ServerName")%>','<%#Eval("ProcessName") %>','<%#Eval("StepLabel") %>',this);">
    ...
    <script type="text/javascript">
        ...
        function openForm(taskId, type, serverName,processName,stepName, ele) {
            ...
            s = window.open('../Ultimus.UWF.Workflow/OpenForm.aspx?TaskId=' + taskId + '&Type=MYTASK&ServerName=' + serverName + '&ProcessName=' + encodeURI(processName) + '&StepName=' + encodeURI(stepName), '', winoption);
        ...
        $(document).ready(function () {
            ...
            $("#frmLink").attr("src", "HomeLink.aspx");
