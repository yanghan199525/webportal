...
{
    public partial class MyTaskListBatch : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                txtProcessName.Text = Request["ProcessName"];
                txtStepName.Text = Request["StepName"];
        ...
        void BindGrid()
        {
            ...
            //获取正在TaskQueue审批的任务，并且过滤
            DataTable dtqueue = DataAccess.Instance("BizDB").ExecuteDataTable("select taskid from WF_TASKQUEUE ");
            lists= lists.FindAll(p => dtqueue.Select("TaskID='" + p.TASKID + "'").Length == 0);
