    ...
    public partial class ButtonList_AddSign : System.Web.UI.UserControl
    {
            ...
            string userName, string summary, Hashtable vars, string tableName, string formID, string documentNo, string type)
        {
            ...
            if (actionType == 0)
            {
                ...
                {
                    addSignCount = ConvertUtil.ToString(DataAccess.Instance("BizDB").ExecuteScalar("select SIGNAPPROVERACCOUNT from WF_ADDSIGN where incident=@incident", incident)).Split(',').Length;
                    sql = @"select count(1) from tasks where processName=@processName and incident=@incident 
                                and steplabel not in ('Begin','Judge','Add Sign End','Complete')  
                                and (status=3 and (steplabel=@steplabel and assignedtouser=@assignedtouser))";
                    taskCount = ConvertUtil.ToInt32(DataAccess.Instance("UltDB").ExecuteScalar(sql, processName, incident, stepLabel, userName.Replace("\\", "/")));
                }
                else
                {
                    addSignCount = ConvertUtil.ToString(DataAccess.Instance("BizDB").ExecuteScalar("select SIGNAPPROVERACCOUNT from WF_ADDSIGN where incident=@incident", incident)).Split(',').Length;
                    sql = @"select count(1) from tasks with(nolock) where processName=@processName and incident=@incident 
                                and steplabel not in ('Begin','Judge','Add Sign End','Complete')  
                                and (status=3 and (steplabel=@steplabel and assignedtouser=@assignedtouser))";
                    taskCount = ConvertUtil.ToInt32(DataAccess.Instance("UltDB").ExecuteScalar(sql, processName, incident, stepLabel, userName.Replace("\\", "/")));
                }

                if (addSignCount == taskCount) //加签的记录与已完成的记录相同
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        {
                            info = _task.ApproveTask("", parenttaskid, APPLICANTACCOUNT, entity.SUMMARY, "", null, formID, processName, incident, stepLabel);
                        }
                        //更新父任务待办任务开始时间
                        if (submitAfter == 0)
                        {
                            DataAccess.Instance("UltDB").ExecuteNonQuery
                                ("update tasks set starttime=@date,endtime=@date1,qstarttime=@date2,qendtime=@date3 where taskid=@taskid",
                                DateTime.Now, DateTime.Now, DateTime.Now, DateTime.Now, parenttaskid);
                        }
                        //更新加签流程状态
                        DataAccess.Instance("BizDB").ExecuteNonQuery("update WF_ADDSIGN set status=2 where incident=@incident", incident);
                    }
                }
            }
            else
