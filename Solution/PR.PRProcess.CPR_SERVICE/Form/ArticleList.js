        ...
        for (var i = 0; i <= tabLen; i++) {
            ...
            if (i == num) {
                if ($("tr:eq(" + i + ")>td:eq(0)").find("input[type = 'checkbox']").val() == "1") {
                    ...
                    rowdata += "\"unittext\":\"" + $("tr:eq(" + i + ")>td:eq(17)").text() + "\",";

                    rowdata += "\"consumptionunit\":\"" + $("tr:eq(" + i + ")>td:eq(16)").text() + "\",";
                    rowdata += "\"consumptionunittext\":\"" + $("tr:eq(" + i + ")>td:eq(17)").text() + "\",";
                    ...
                    rowdata += "\"articleid\":\"" + $("tr:eq(" + i + ")>td:eq(28)").text() + "\",";
...
$("input[type='checkbox']").on("change", function () {
    ...
    if ($(this).is(":checked")) {
        ...
        var FamilyCode = tdList.nextAll().eq(8).text();
        $("#supplerName").empty();
        $("#supplerName").selectpicker('val', SupplerCode);
        $("#supplerName").append("<option value=''selected>" + SupplerCode + "</option>");
        $("#supplerName").selectpicker('refresh');
...
//加载该询报价单下的授权供应商
function BindSuppler(searchRFQInput, searchSupper, searchArticleName) {
    $.ajax({
        type: "post",
        datatype: "json",
        contentType: "application/json",
