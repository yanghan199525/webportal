...
function BindSupplier() {
    ...
    $.ajax({
        ...
        data: datadata,
        //data: "{\"suppliertype\":\"" + suppliertype + "\",\"searchcondition\":\"" + searchSupplier + "\",\"pccode\":\"" + $('#hdSiteCode').val() + "\",\"subfamilycode\":\"" + $('#ddlSubSubFamily').val() + "\"}",
        ...
        success: function (data) {
            if (data.d != "") {
                ...
                if ($("#hdSupplierCode").val() != "") {
                    ...
                    if (suppliertype == "9") {
                        //alert($("#hdAuthorizedSupplierCode").val());
...
function BindArticle() {
    ...
    $.ajax({
        ...
        //data: "{\"sitecode\":\"" + siteCode + "\",\"articlefamily\":\"" + articlefamily + "\",\"suppliertype\":\"" + suppliertype + "\",\"suppliercode\":\"" + suppliercode + "\"}",
        data: "{\"sitecode\":\"" + siteCode + "\",\"articlefamily\":\"" + articlefamily + "\",\"suppliertype\":\"" + suppliertype + "\",\"suppliercode\":\"" + suppliercode + "\",\"category\":\"" + category + "\",\"familycode\":\"" + familycode + "\",\"searchcondition\":\"" + searchArticles + "\"}",
        success: function (data) {
            if (data.d != "") {
                var arrData = data.d.replace(/[\s\n]/g, " ").replace(/[\r]/g, "\\r")
...
function returnValue1() {
    ...
    if (!flag) {
        ...
        //else if (suppliervalue == "" || suppliervalue == null) {
        //    if (language.toLowerCase() == "en-us") {
        //        alert('Tip : The supplier cannot be empty');
        //    }
        //    else {
...
function ClearDialog() {
    ...
    ddlFamily.selectpicker('val', familycode);
    ddlSupplier.prop('disabled', 'true');
    ddlFamily.prop('disabled', 'true');
    ddlSupplier.selectpicker('refresh');
    ddlFamily.selectpicker('refresh');
