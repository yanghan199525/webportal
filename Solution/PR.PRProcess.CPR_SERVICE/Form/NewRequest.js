...
$(function () {
    ...
    if (Type == "MYTASK") {
        ...
        var tabCtl = document.getElementById("tb_CPR_SERVICE_ITEMS");
    ...
    //var h_height = $("#fld_SITENAME").parent().height();
    //$("#fld_DELIVERYDATE").height(h_height-3);
    ...
    for (var i = 1; i < tabCtl.rows.length; i++) {
        ...
        if (arrData != undefined && arrData.length>0) {
            for (var i = 0; i < arrData.length; i++) {
                if (arrData[i].ArticleCode == articleCode) {
...
function futureFinishTime(field, rules, i, options) {
    debugger
    var InputTime = new Date($("#fld_ServiceEstimatedFinishTime").val());
    var datetime = new Date($("#hdFinshDate").val());
    if (InputTime <datetime) {
        options.allrules.validate2fields.alertText = "服务预计完成时间必须大于等于当天<br/>The estimated service completion time must be greater than the current day";
        return options.allrules.validate2fields.alertText;
    }
}
...
function AddCprArticle(tabId, arrData) {
    ...
    for (var i = 0; i < arrData.length; i++) {
        if (tabCtl.rows.length >= 2 && $(FirstRow).find("input[id*='fld_APPLYREASON']").val() != "") {
            ...
            $(newRow).find("input[id*='InvoiceType']").val(arrData[i].InvoiceType);
            ...
            $("#tb_CPR_SERVICE_ITEMS_rowCount").val(rowIndex + 1);
            if (isIE()) {
            ...
            var existrow = tabCtl.rows[tabCtl.rows.length - 1];
            $("#fld_SUPPLIERCODE").val(arrData[i].suppliercode);
            ...
            $(existrow).find("input[id*='InvoiceType']").val(arrData[i].InvoiceType);
...
function addCPRITEMRow(tabId, res) {
    amount = $.trim($('#fld_AMOUNT').val()) == "" ? "0" : $.trim($('#fld_AMOUNT').val());
    try {
        var tabCtl = document.getElementById(tabId);
        var existrow = tabCtl.rows[tabCtl.rows.length - 1];
        if (tabCtl.rows.length == 2 && $(existrow).find("input[id*='fld_APPLYREASON']").val() == "") {
            ...
            $(existrow).find("input[id*='InvoiceType']").val(res.InvoiceType);
            ...
            if (!checkCPRItems(tabId, res)) {
                ...
                $(newRow).find("input[id*='InvoiceType']").val(res.InvoiceType);
...
function SumAmount() {
    ...
    for (var i = 1; i < tabCtl.rows.length; i++) {
        ...
        $(existrow).find("input[id*='fld_SUBTOTALAMOUNT']").val(siteprice * orderquantity);
        var subtotalamount = $(existrow).find("input[id*='fld_SUBTOTALAMOUNT']").val();
        amount = calculatenumber(subtotalamount, amount, 1);
    }
    $("#fld_AMOUNT").val(thousands(amount));
    //if (thousands(amount) != thousands($("#fld_ORIGINALAMOUNT").text())) {
    //    $("#fld_AMOUNT").css("color", "red");
    //    $("#fld_ISSALARY").val(1);
    //} else {
    //    $("#fld_ISSALARY").val(0);
    //}
...
//修改为可输入整数、大于0且带有一位小数、大于0且带有两位小数  Hour Meter Square meter
...
//            var existing_siteprice = $(existing_row).find("input[id*='fld_SITEPRICE']").val();
//            if (articlecode == existing_articlecode && siteprice == existing_siteprice) {
//                var new_orderquantity = Number(orderquantity) + Number(existing_orderquantity);
//                $(existing_row).find("input[id*='fld_ORDERQUANTITY']").val(new_orderquantity);
//                return true;
//            }
//            else {
...
//            if (articlename == exist_articlename && orderunit == exist_orderunit && unit == exist_unit && consumptionunit == exist_consumptionunit && conversion == exist_conversion && stock == exist_stock && subsubfamilycode == exist_subsubfamilycode) {
//                debugger
//                var news_orderquantity = Number(orderquantity) + Number(exist_orderquantity);
//                $(exist_rows).find("input[id*='fld_ORDERQUANTITY']").val(news_orderquantity);
//                return true;
//            }
//            else {
...
function checkArticleCode() {
    ...
    if (suppliertype == "9") {
        ...
        for (var i = 1; i < tabCtl.rows.length; i++) {
            ...
            var existing_articlecode = $(existing_row).find("input[id*='fld_ARTICLECODE']").val();
            var existing_siteprice = $(existing_row).find("input[id*='fld_SITEPRICE']").val();
            var existing_rownos = $(existing_row).find("input[id*='fld_ROWNO']").val();
            for (var j = 1; j < tabCtl.rows.length; j++) {
                if (i != j) {
                    ...
                    var existing_j_rownos = $(existing_j_row).find("input[id*='fld_ROWNO']").val();
                    if (existing_articlecode == existing_j_articlecode && existing_siteprice == existing_j_siteprice) {
                        m++;
                    }
                    if (existing_rownos == existing_j_rownos) {
                        no++;
                    }
                }
                else {
...
/**
 * Add By Sylvia At 2020-08-04
 * 判断是否为代采购，是则隐藏固定资产选择框
...
function changeApplyPurpose(init) {
    ...
    if (ApplyPurpose == "2") {
        ...
        //alert('客户代采购提示信息窗口，默认一级加签人，附件变更为必填项，无附件无法进行提交');
...
function changeSupplierType() {
    ...
    var SupplierType = $("#fld_SUPPLIERTYPE").val();
    if (SupplierType == "9") {
        //$("#AttachmentsAdd").style.display = "block";
...
function isShow() {
    ...
    //    }
    //    const targetChar = 'fld_SITEPRICE';
    //    // 使用 querySelectorAll 方法和属性选择器查找元素
    //    const elements = document.querySelectorAll(`[id*="${targetChar}"]`);
    //    // 遍历找到的元素并输出其 id
    ...
    //    const targetChar1 = 'fld_ORDERQUANTITY';
    //    // 使用 querySelectorAll 方法和属性选择器查找元素
    //    const elements1 = document.querySelectorAll(`[id*="${targetChar1}"]`);
    //    // 遍历找到的元素并输出其 id
...
function initInvoiceLinks() {
    ...
    $("#tb_CPR_SERVICE_ITEMS tbody tr td.td_INVOICEPATH [data-field='INVOICEPATH']").each(function () {
...
function showInvoiceInfo() {
    ...
    if ($("#fld_SUPPLIERTYPE").val() == "5") {
        ...
        config.tableCols.forEach(function (colClass) {
            $(`#tb_CPR_SERVICE_ITEMS thead tr td.${colClass}, #tb_CPR_SERVICE_ITEMS tbody tr td.${colClass}`).show();
        });
    }
    else {
        // 隐藏div
        ...
        config.tableCols.forEach(function (colClass) {
            $(`#tb_CPR_SERVICE_ITEMS thead tr td.${colClass}, #tb_CPR_SERVICE_ITEMS tbody tr td.${colClass}`).hide();
        });
    }
