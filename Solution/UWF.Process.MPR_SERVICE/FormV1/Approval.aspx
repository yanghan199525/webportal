                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        <table id="tb_MMPR_SERVICE_ITEMS"
                            ...
                            <tbody>
                                <%--服务端变量，记录上一条日期与颜色标记，放在Repeater外面--%>
                                <% string lastDeliveryDate = null; int groupColorFlag = 0; %>
                                <ult:Repeater ID="read_detail_PROC_MMPR_SERVICE_ITEMS" runat="server">
                                    <itemtemplate>
                    <%
                        string currDate = Eval("DELIVERYDATE")?.ToString();
                        if (currDate != lastDeliveryDate)
                        {
                            groupColorFlag = groupColorFlag == 0 ? 1 : 0;
                            lastDeliveryDate = currDate;
                        }
                        string trBg = groupColorFlag == 1 ? "#f2f7ff" : "#ffffff";
                    %>
                    <tr style="background-color:<%=trBg%>">
                        ...
                        <td data-label='<%=Lang.Get("No").Split('<')[0] %>'>
                            ...
                            <ult:TextBox ID="fld_ROWGUID"
                                         data-field="ROWGUID"
                                         CssClass="index hidden"
                                         runat="server"
                                         ControlValue='<%#Eval("ROWGUID")%>'>
                            </ult:TextBox>
                        </td>
                        <td class="hidden td_ARTICLECODE"
