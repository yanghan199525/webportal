    ...
    public partial class AddPRItemPage : System.Web.UI.Page
    {
        ...
        public static string BindFamily(string categorycode)
        {
            ...
            if (language == "ZH-CN")
            {
                ...
                {
                    dt.Rows[i]["FamilyNameCN"] = ToTraditional(dt.Rows[i]["FamilyNameCN"].ToString());
                }
            }
        ...
        public static string BindSubFamily(string categorycode, string familycode)
        {
            ...
            if (language == "ZH-CN")
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    dt.Rows[i]["SubFamilyNameCN"] = ToTraditional(dt.Rows[i]["SubFamilyNameCN"].ToString());
                }
            }
        ...
        public static string BindSubSubFamily(string categorycode, string familycode, string subfamilycode)
        {
            ...
            if (language == "ZH-CN")
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    dt.Rows[i]["SubSubFamilyNameCN"] = ToTraditional(dt.Rows[i]["SubSubFamilyNameCN"].ToString());
                }
            }
        ...
        public static string BindSupplier(string suppliertype, string searchcondition, string pccode, string subfamilycode)
        {
               ...
               if (suppliertype == "2" || suppliertype == "5")
            {
                ...
                sqlBuilder.AppendFormat(
                    "SELECT TOP 20 SupplierCode, SupplierName{0} " +
                    "FROM [dbo].[SODEXO_t_Foun_SCM_Suppliers] " +
                    "WHERE ",
                    language);
                ...
                if (suppliertype == "2")
                {
                    sqlBuilder.Append("IndustryKey = 'V002'");
                }
                else
                {
                    sqlBuilder.Append("SupplierAccountGroup IN ('E101', 'CASH')");
                }
               ...
               if (!string.IsNullOrWhiteSpace(searchcondition))
                {
                    ...
                    //parameters.Add("@searchcondition", searchcondition);
                }
        ...
        public static string BindOrderUnit()
        {
            ...
            if (language == "ZH-CN")
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    dt.Rows[i]["OrderUnitCN"] = ToTraditional(dt.Rows[i]["OrderUnitCN"].ToString());
                }
            }
        ...
        public static string BindBaseUnit()
        {
            ...
            if (language == "ZH-CN")
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    dt.Rows[i]["BaseUnitCN"] = ToTraditional(dt.Rows[i]["BaseUnitCN"].ToString());
                }
            }
