    ...
    public partial class Approval : System.Web.UI.Page
    {
        ...
        public string getDeliveryDate(string ProcessName, string Incident, string TaskID)
        {
            ...
            try
            {
                ...
                {
                    DataTable PROC_HK_CPR_SERVICE = DataAccess.Instance("BizDB").ExecuteDataTable("SELECT * FROM PROC_HK_CPR_SERVICE WHERE PROCESSNAME='" + ProcessName + "' AND INCIDENT=" + Incident);
                    if (PROC_HK_CPR_SERVICE != null)
                    {
                        DeliveryDate = PROC_HK_CPR_SERVICE.Rows[0]["DELIVERYDATE"].ToString();
                    }
                }
                return DeliveryDate;
            }
