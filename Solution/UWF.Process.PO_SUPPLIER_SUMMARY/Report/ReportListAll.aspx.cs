...
namespace UWF.Process.CAPEX_NONFOOD
{
    public partial class ReportListAll : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            cprt.Source = string.Format(@"BizDB.SELECT TOP 200
                AAA.[FORMID],
                AAA.[PROCESSNAME],
                AAA.[INCIDENT],
                AAA.[DOCUMENTNO],
                ...
                AAA.[SUPPLIERCODE],
                AAA.[SUPPLIERNAME],
                WF.[ACTION]  -- 审批历史中的操作类型
            FROM (
                -- 子查询AAA：合并两表数据并筛选排序
                SELECT TOP 200 
                    *
                FROM (
                    -- 基础数据查询1：提取PROC_CAPEX_NONFOOD表数据
                    SELECT 
                        [FORMID],
                        [PROCESSNAME],
                        [INCIDENT],
                    ...
                    WHERE 1 = 1 
                        AND SITECODE = '{0}' 
                        AND APPLICANTCODE = '{1}'
            		    AND REQUESTDATE > GETDATE() - 60

                    UNION ALL  -- 合并两个表的数据

                    -- 基础数据查询2：提取PROC_CAPEX_SERVICE表数据
                    SELECT 
                        [FORMID],
                        [PROCESSNAME],
                        [INCIDENT],
                ...
                -- 过滤条件：再次确认站点和申请人（冗余过滤，确保数据准确性）
                WHERE 1 = 1 
                    AND T.SITECODE = '{0}' 
                    AND T.APPLICANTCODE = '{1}'
                -- 排序规则：按申请日期倒序，优先获取最新记录
                ORDER BY T.REQUESTDATE DESC
            ) AAA
            -- 关联审批历史表：获取每个表单的最新审批记录
            INNER JOIN (
                SELECT 
        FORMID,
        [ACTION],
