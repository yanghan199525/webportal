...
namespace UWF.Process.HK_CAPEX_NONFOOD
{
    ...
    WF.[ACTION]
FROM (
    -- 1. 使用 UNION ALL 合并两表数据，过滤条件完全下推
    SELECT 
    ...
    SELECT 
        ...
        [APPROVE], [APPLICANTTEL], [COMPANY], [COSTCENTER], 
        '' AS [APPREMARK], [AMOUNT], [SUPPLIERCODE], [SUPPLIERNAME]
    FROM PROC_HK_CAPEX_SERVICE
    WHERE SITECODE = '{0}' 
      AND APPLICANTCODE ='{1}' 
     -- AND REQUESTDATE > DATEADD(DAY, -60, GETDATE())
) AS AAA
-- 2. 使用 CROSS APPLY 高效获取每个表单的最新审批记录，彻底替代 ROW_NUMBER()
CROSS APPLY (
    SELECT TOP (1) 
        ACTION
    FROM WF_APPROVALHISTORY
    WHERE FORMID = AAA.FORMID 
      AND PROCESSNAME IN ('HK_CAPEX_SERVICE', 'HK_CAPEX_NONFOOD')
...
WHERE AAA.INCIDENT <> -1", pcCode, empNo
        ...
        protected void lbExport_Click(object sender, EventArgs e)
        {
            ...
            dt = ExportLogic.GetSchemaTable("HK_CAPEX_NONFOOD", dt);
            ExcelUtil.Export(dt);
        }
