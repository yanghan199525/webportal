...
namespace PR.PRProcess.HK_CPR_FOOD
{
    ...
    {
        private int oldCount = 200000;
        private int newCount = 20000000;
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            rpt.Source = string.Format(@"BizDB.select * from (SELECT T.*,WF.ACTION FROM (
		            ...
		            FROM PROC_HK_CPR_FOOD 
		            UNION SELECT FORMID, PROCESSNAME, INCIDENT, DOCUMENTNO, CREATEBY, CREATEBYACCOUNT,
		            CREATEBYCODE, APPLICANT, APPLICANTACCOUNT, APPLICANTCODE, APPLICANTTEL, REQUESTDATE, COMPLETEDATE,
		            DEPARTMENT, DEPARTMENTID, COMPANY, COSTCENTER, PROCESSSUMMARY, STATUS, APPLYPURPOSE, SUPPLIERTYPE,
		            SITECODE, SITENAME, DELIVERYDATE, SUPPLIERCODE, SUPPLIERNAME, ASSETTYPE, AMOUNT, APPREMARK, APPROVEDATE,
		            PCCOMPCODE, APPLYPURPOSETXT, SUPPLIERTYPETXT, ASSETTYPETXT, CPRFAMILYCODE, OVERTIME, PURCHASINGAGENT,
		            ONLINEORSUPERMARKET, USER_SIGNEDAPPROVER, USER_SIGNEDAPPROVER2, USER_SIGNEDAPPROVER3, USER_SIGNEDAPPROVERNAME,
		            USER_SIGNEDAPPROVER2NAME, USER_SIGNEDAPPROVER3NAME, SIGNEDAPPROVERNUMBER, DELIVERY, APPROVE 
		            FROM PROC_HK_CPR_NONFOOD where  INCIDENT!='-1' 
		            UNION SELECT FORMID, PROCESSNAME, INCIDENT, DOCUMENTNO, CREATEBY, CREATEBYACCOUNT,
		            CREATEBYCODE, APPLICANT, APPLICANTACCOUNT, APPLICANTCODE, APPLICANTTEL, REQUESTDATE, COMPLETEDATE,
		            DEPARTMENT, DEPARTMENTID, COMPANY, COSTCENTER, PROCESSSUMMARY, STATUS, APPLYPURPOSE, SUPPLIERTYPE, SITECODE, SITENAME, DELIVERYDATE, SUPPLIERCODE,
		            SUPPLIERNAME, ASSETTYPE, AMOUNT, APPREMARK, APPROVEDATE, PCCOMPCODE, APPLYPURPOSETXT, SUPPLIERTYPETXT, ASSETTYPETXT, CPRFAMILYCODE, OVERTIME, PURCHASINGAGENT,
		            ONLINEORSUPERMARKET, USER_SIGNEDAPPROVER, USER_SIGNEDAPPROVER2, USER_SIGNEDAPPROVER3, USER_SIGNEDAPPROVERNAME, USER_SIGNEDAPPROVER2NAME, USER_SIGNEDAPPROVER3NAME, 
		            SIGNEDAPPROVERNUMBER, DELIVERY, APPROVE FROM PROC_HK_CPR_SERVICE where  INCIDENT!='-1' 
	            ) T inner join (SELECT * FROM WF_APPROVALHISTORY WHERE ID IN(SELECT TOP " + newCount + " MAX(ID) ID FROM WF_APPROVALHISTORY GROUP BY FORMID)) WF ON T.FORMID=WF.FORMID ) a");
...
FROM
    ...
    FROM
        ...
        SELECT FORMID,
         ...
         APPROVE
        FROM PROC_HK_CPR_SERVICE
        WHERE INCIDENT != '-1') T,
            (SELECT *
                FROM PROC_CPR_LOG CPR_LOG
                WHERE EXT01 = CPR_LOG.EXT01
                        AND EXT02 = CPR_LOG.EXT02
                        AND CREATEDATE > GETDATE() - 30) CL,
                    (SELECT *
                    FROM WF_APPROVALHISTORY
                    WHERE ID IN (SELECT TOP " + newCount + " MAX(ID) ID FROM WF_APPROVALHISTORY GROUP BY  FORMID)) WF WHERE T.PROCESSNAME = CL.EXT01 AND T.INCIDENT = CL.EXT02  AND CL.SYNCSTATUS = 0 AND T.FORMID = WF.FORMID AND T.DELIVERYDATE > GETDATE() - 30) a ");
