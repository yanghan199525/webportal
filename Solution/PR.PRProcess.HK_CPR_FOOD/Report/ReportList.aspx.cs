...
namespace PR.PRProcess.HK_CPR_FOOD
{
    public partial class ReportList : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            var oldCount = 200;
            var newCount = 200;
            rpt.Source = string.Format(@"BizDB.select T.FORMID,
          T.PROCESSNAME,
          T.INCIDENT,
          DOCUMENTNO,
          CREATEBY,
          ...
          ONLINEORSUPERMARKET,
          USER_SIGNEDAPPROVER,
          USER_SIGNEDAPPROVER2,
          USER_SIGNEDAPPROVER3,
          USER_SIGNEDAPPROVERNAME,
          USER_SIGNEDAPPROVER2NAME,
          USER_SIGNEDAPPROVER3NAME,
          SIGNEDAPPROVERNUMBER,
          DELIVERY,
          APPROVE,
		  ACTION 
		  from(
  ...
  (
   select  * from WF_APPROVALHISTORY as a 
                  where ID = (select max(b.ID) 
           from WF_APPROVALHISTORY as b 
            where a.FORMID = b.FORMID)
			) wf
on T.FORMID = WF.FORMID AND T.INCIDENT != '-1')", pcCode, empNo);
            ...
            drafts.Source = string.Format("BizDB.SELECT TOP " + oldCount + " AAA.FORMID, AAA.PROCESSNAME, AAA.INCIDENT, DOCUMENTNO, AAA.CREATEBY, CREATEBYACCOUNT, CREATEBYCODE, APPLICANT, APPLICANTACCOUNT, APPLICANTCODE, APPLICANTTEL, REQUESTDATE, COMPLETEDATE, DEPARTMENT, DEPARTMENTID, COMPANY, COSTCENTER, PROCESSSUMMARY, AAA.STATUS, APPLYPURPOSE, SUPPLIERTYPE, SITECODE, SITENAME, DELIVERYDATE, SUPPLIERCODE, SUPPLIERNAME, ASSETTYPE, AMOUNT, APPREMARK, APPROVEDATE, PCCOMPCODE, APPLYPURPOSETXT, SUPPLIERTYPETXT, ASSETTYPETXT, CPRFAMILYCODE, OVERTIME, PURCHASINGAGENT, ONLINEORSUPERMARKET, USER_SIGNEDAPPROVER, USER_SIGNEDAPPROVER2, USER_SIGNEDAPPROVER3, USER_SIGNEDAPPROVERNAME, USER_SIGNEDAPPROVER2NAME, USER_SIGNEDAPPROVER3NAME, SIGNEDAPPROVERNUMBER, DELIVERY, APPROVE FROM (SELECT TOP " + newCount + " * FROM (SELECT FORMID, PROCESSNAME, INCIDENT, DOCUMENTNO, CREATEBY, CREATEBYACCOUNT, CREATEBYCODE, APPLICANT, APPLICANTACCOUNT, APPLICANTCODE, APPLICANTTEL, REQUESTDATE, COMPLETEDATE, DEPARTMENT, DEPARTMENTID, COMPANY, COSTCENTER, PROCESSSUMMARY, STATUS, APPLYPURPOSE, SUPPLIERTYPE, SITECODE, SITENAME, DELIVERYDATE, SUPPLIERCODE, SUPPLIERNAME, ASSETTYPE, AMOUNT, APPREMARK, APPROVEDATE, PCCOMPCODE, APPLYPURPOSETXT, SUPPLIERTYPETXT, ASSETTYPETXT, CPRFAMILYCODE, OVERTIME, PURCHASINGAGENT, ONLINEORSUPERMARKET, USER_SIGNEDAPPROVER, USER_SIGNEDAPPROVER2, USER_SIGNEDAPPROVER3, USER_SIGNEDAPPROVERNAME, USER_SIGNEDAPPROVER2NAME, USER_SIGNEDAPPROVER3NAME, SIGNEDAPPROVERNUMBER, DELIVERY, APPROVE FROM PROC_HK_CPR_FOOD UNION SELECT FORMID, PROCESSNAME, INCIDENT, DOCUMENTNO, CREATEBY, CREATEBYACCOUNT, CREATEBYCODE, APPLICANT, APPLICANTACCOUNT, APPLICANTCODE, APPLICANTTEL, REQUESTDATE, COMPLETEDATE, DEPARTMENT, DEPARTMENTID, COMPANY, COSTCENTER, PROCESSSUMMARY, STATUS, APPLYPURPOSE, SUPPLIERTYPE, SITECODE, SITENAME, DELIVERYDATE, SUPPLIERCODE, SUPPLIERNAME, ASSETTYPE, AMOUNT, APPREMARK, APPROVEDATE, PCCOMPCODE, APPLYPURPOSETXT, SUPPLIERTYPETXT, ASSETTYPETXT, CPRFAMILYCODE, OVERTIME, PURCHASINGAGENT, ONLINEORSUPERMARKET, USER_SIGNEDAPPROVER, USER_SIGNEDAPPROVER2, USER_SIGNEDAPPROVER3, USER_SIGNEDAPPROVERNAME, USER_SIGNEDAPPROVER2NAME, USER_SIGNEDAPPROVER3NAME, SIGNEDAPPROVERNUMBER, DELIVERY, APPROVE FROM PROC_HK_CPR_NONFOOD UNION SELECT FORMID, PROCESSNAME, INCIDENT, DOCUMENTNO, CREATEBY, CREATEBYACCOUNT, CREATEBYCODE, APPLICANT, APPLICANTACCOUNT, APPLICANTCODE, APPLICANTTEL, REQUESTDATE, COMPLETEDATE, DEPARTMENT, DEPARTMENTID, COMPANY, COSTCENTER, PROCESSSUMMARY, STATUS, APPLYPURPOSE, SUPPLIERTYPE, SITECODE, SITENAME, DELIVERYDATE, SUPPLIERCODE, SUPPLIERNAME, ASSETTYPE, AMOUNT, APPREMARK, APPROVEDATE, PCCOMPCODE, APPLYPURPOSETXT, SUPPLIERTYPETXT, ASSETTYPETXT, CPRFAMILYCODE, OVERTIME, PURCHASINGAGENT, ONLINEORSUPERMARKET, USER_SIGNEDAPPROVER, USER_SIGNEDAPPROVER2, USER_SIGNEDAPPROVER3, USER_SIGNEDAPPROVERNAME, USER_SIGNEDAPPROVER2NAME, USER_SIGNEDAPPROVER3NAME, SIGNEDAPPROVERNUMBER, DELIVERY, APPROVE FROM PROC_HK_CPR_SERVICE) T WHERE 1 = 1 AND T.SITECODE = '{0}' AND T.APPLICANTCODE='{1}' ORDER BY T.REQUESTDATE DESC) AAA,(SELECT * FROM WF_DRAFT) WF_D WHERE AAA.FORMID=WF_D.FORMID AND AAA.INCIDENT='-1' ORDER BY AAA.REQUESTDATE DESC", pcCode, empNo);
            ...
            importFailedRecord.Source = string.Format(@"BizDB.SELECT
        MAX(h.FORMID) FORMID ,
         MAX(h.PROCESSNAME) PROCESSNAME,                                           
         MAX(h.INCIDENT) INCIDENT,
         MAX(DOCUMENTNO)DOCUMENTNO,
         MAX(CREATEBY)CREATEBY,
         MAX(CREATEBYACCOUNT)CREATEBYACCOUNT,
         MAX(CREATEBYCODE)CREATEBYCODE,
         MAX(APPLICANT)APPLICANT,
         MAX(APPLICANTACCOUNT)APPLICANTACCOUNT,
         MAX(APPLICANTCODE)APPLICANTCODE,
         MAX(APPLICANTTEL)APPLICANTTEL,
         MAX(REQUESTDATE)REQUESTDATE,
         MAX(COMPLETEDATE)COMPLETEDATE,
         MAX(DEPARTMENT)DEPARTMENT,
         MAX(DEPARTMENTID)DEPARTMENTID,
         MAX(COMPANY)COMPANY,
         MAX(COSTCENTER)COSTCENTER,
         MAX(PROCESSSUMMARY)PROCESSSUMMARY,
         MAX(h.STATUS)STATUS,
         MAX(APPLYPURPOSE)APPLYPURPOSE,
         MAX(SUPPLIERTYPE)SUPPLIERTYPE,
         MAX(SITECODE)SITECODE,
         MAX(SITENAME)SITENAME,
         MAX(DELIVERYDATE)DELIVERYDATE,
         MAX(SUPPLIERCODE)SUPPLIERCODE,
         MAX(SUPPLIERNAME)SUPPLIERNAME,
         MAX(ASSETTYPE)ASSETTYPE,
         MAX(AMOUNT)AMOUNT,
         MAX(APPREMARK)APPREMARK,
         MAX(APPROVEDATE)APPROVEDATE,
         MAX(PCCOMPCODE)PCCOMPCODE,
         MAX(APPLYPURPOSETXT)APPLYPURPOSETXT,
         MAX(SUPPLIERTYPETXT)SUPPLIERTYPETXT,
         MAX(ASSETTYPETXT)ASSETTYPETXT,
         MAX(CPRFAMILYCODE)CPRFAMILYCODE,
         MAX(OVERTIME)OVERTIME,
         MAX(PURCHASINGAGENT)PURCHASINGAGENT,
         MAX(ONLINEORSUPERMARKET)ONLINEORSUPERMARKET,
         MAX(USER_SIGNEDAPPROVER)USER_SIGNEDAPPROVER,
         MAX(USER_SIGNEDAPPROVER2)USER_SIGNEDAPPROVER2,
         MAX(USER_SIGNEDAPPROVER3)USER_SIGNEDAPPROVER3,
         MAX(USER_SIGNEDAPPROVERNAME)USER_SIGNEDAPPROVERNAME,
         MAX(USER_SIGNEDAPPROVER2NAME)USER_SIGNEDAPPROVER2NAME,
         MAX(USER_SIGNEDAPPROVER3NAME)USER_SIGNEDAPPROVER3NAME,
         MAX(SIGNEDAPPROVERNUMBER)SIGNEDAPPROVERNUMBER,
         MAX(DELIVERY)DELIVERY,
         MAX(APPROVE)APPROVE,
		 MAX(WF.CREATEDATE)CREATEDATE,
		 MAX(ERRORMSG)ERRORMSG		
        FROM
          PROC_HK_CPR_FOOD h
          inner join
             PROC_CPR_LOG WF
         on  h.PROCESSNAME = WF.EXT01 
		 and h.INCIDENT=WF.EXT02
        AND h.SITECODE = '{0}'
        AND h.APPLICANTCODE = '{1}'
         AND WF.SYNCSTATUS = 0
		GROUP BY  WF.EXT01,WF.EXT02
		having MAX(WF.CREATEDATE)>DATEADD(dd,-30,GETDATE())
       UNION
	   SELECT
           MAX(h.FORMID) FORMID ,
         MAX(h.PROCESSNAME) PROCESSNAME,                                           
	   ...
	   SELECT
        ...
        AND WF.SYNCSTATUS = 0
		GROUP BY  WF.EXT01,WF.EXT02
		having MAX(WF.CREATEDATE)>DATEADD(dd,-30,GETDATE())", pcCode, empNo);
        importFailedRecord.Sort = " REQUESTDATE DESC";
        ...
        protected void lbExport_Click(object sender, EventArgs e)
        {
            ...
            dt = ExportLogic.GetSchemaTable("HK_CPR_FOOD", dt);
            ExcelUtil.Export(dt);
        }
        ...
        protected void lbDraftDetails_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string url = string.Format("{6}/Portal/Ultimus.UWF.Home.V3/Workflow/OpenForm.aspx?TaskId={0}&Type={1}&FORMID={2}&incident={3}&ProcessName={4}&StepName={5}", taskid, "Draft", formid, "-1", processname, "Begin", rootPath);
