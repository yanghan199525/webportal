...
{
    public partial class AssociatedProcess : System.Web.UI.UserControl
    {
        ...
        void BindAttachments()
        {
            try
            {
                ...
                DataTable dt = GetAssociatedProcessByFormID(userInfo.FormID, txtType.Text);
        ...
        protected void Repeater1_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "Delete")
                {
                    try
                    {
                        if (DeleteAssociatedProcessByID(e.CommandArgument.ToString()))
                        {
                            BindAttachments();
                        }
                        else
        ...
        #region 辅助方法
        public virtual DataTable GetAssociatedProcessByFormID(string strFormid, string type)
        {
            try
            {
                string sql = "select * from WF_ASSOCIATEDPROCESS where FORMID=@FORMID";
                return DataAccess.Instance("BizDB").ExecuteDataTable(sql, strFormid);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public virtual bool DeleteAssociatedProcessByID(string ID)
        {
            try
            {
                ...
                if (ID.IndexOf(",") < 0)
                {
                    sql = "delete from WF_Attachment where NEWGUID=@ID";
                    if (DataAccess.Instance("BizDB").ExecuteNonQuery(sql, ID) > 0)
                        return true;
                    else
                        return false;
                }
                ...
                {
                    sql = "delete from WF_Attachment where NEWGUID in (" + ID + ")";
