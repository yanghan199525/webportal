...
FROM
    ...
    INNER JOIN [dbo].SODEXO_t_Foun_SCM_FamilyHierarchy fh ON a.ArticleFamily = CONVERT(nvarchar(20),CONVERT(decimal(20,0), fh.SubSubFamilyCode ))
WHERE
    [SiteCode] = '{0}'
    ...
    SubSubFamilyTextCN FROM [dbo].[SODEXO_Article] a  INNER JOIN [dbo].SODEXO_t_Foun_SCM_FamilyHierarchy fh ON a.ArticleFamily = CONVERT(nvarchar(20),CONVERT(decimal(20,0), fh.SubSubFamilyCode )) WHERE [SiteCode]='{0}' AND [ArticleFamily]='{1}' ORDER BY [ArticleName]", sitecode, articlefamily);
                //dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
            }
            dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
