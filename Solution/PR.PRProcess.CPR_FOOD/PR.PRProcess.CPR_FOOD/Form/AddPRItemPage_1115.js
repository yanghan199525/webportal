...
$(function () {
    ...
    //$('#ddlOrderUnit').change(function () {
    //    $('#ddlUnit').val($('#ddlOrderUnit').val());
    //    $('#ddlConsumptionUnit').val($('#ddlOrderUnit').val());
...
function BindSubSubFamilyCE() {
    ...
    $.ajax({
        ...
        async: false,
        url: 'AddPRItemPageHandler.ashx',
        //data: "{\"SubSubFamilyCode\":\"" + $('#ddlSubSubFamily').val() + "\"}",
        data: { username: username, SubSubFamilyCode: $('#ddlSubSubFamily').val(), Method: "BindSubSubFamilyCE" },
...
function BindSupplier() {
    ...
    var datadata = "";
    if (suppliertype == "9") {
        datadata = "{\"suppliertype\":\"" + suppliertype + "\",\"searchcondition\":\"" + $("#hdAuthorizedSupplierCode").val() + "\",\"pccode\":\"" + $('#hdSiteCode').val() + "\",\"subfamilycode\":\"" + $('#ddlSubSubFamily').val() + "\"}";
    }
    else {
    ...
    $.ajax({
        ...
        url: 'AddPRItemPageHandler.ashx',
        //data: datadata,
        //data: { username: username, datadata: datadata, Method: "BindSupplier" },
        data: { username: username, suppliertype: suppliertype, searchcondition: searchSupplier, sitecode: $('#hdSiteCode').val(), subfamilycode: $('#ddlSubSubFamily').val(), Method: "BindSupplier" },
                ...
                if ($("#hdSupplierCode").val() != "") {
                    ...
                    if (suppliertype == "9") {
                        if ($("#ddlSupplier option").length > 1) {
                            ...
                            //ddlSupplier.selectpicker('refresh');
							for (var i = 0; i < arrData.length; i++) {
                                if (arrData[i].SupplierCode.trim() == $("#hdAuthorizedSupplierCode").val().trim()) {
                                    ddlSupplier.empty();
                                    ddlSupplier.selectpicker('val', arrData[0].SupplierNameCN );
                                    ddlSupplier.append("<option value=''selected>" + arrData[0].SupplierNameCN  + "</option>");
                                    ddlSupplier.selectpicker('refresh');
                                    ddlSupplier.find("option:selected").val(arrData[0].SupplierNameCN );								
                                }	
                            }
...
function BindArticle() {
    ...
    $.ajax({
        ...
        data: { username: username, sitecode: siteCode, articlefamily: articlefamily, suppliertype: suppliertype, suppliercode: suppliercode, categorycode: category, searchcondition: searchArticles, Method: "BindArticle" },
        success: function (data) {
...
function BindFamily(category) {
    ...
    $.ajax({
        ...
        datatype: "json",
        //contentType: "application/json",
        url: "AddPRItemPageHandler.ashx",//'/BindFamily',
        data: { username: username, categorycode: category, Method: "BindFamily" },//"{\"categorycode\":\"" + category + "\"}",
        //data: function () { return { 'selectedCategory': selectedCategory } },
        success: function (data) {
            debugger
...
function BindSubFamily(category, family) {
    ...
    $.ajax({
        ...
        data: { username: username, categorycode: category, familycode: family, Method: "BindSubFamily" },
        success: function (data) {
...
function BindSubSubFamily(category, family, subfamily) {
    ...
    $.ajax({
        ...
        data: { username: username, categorycode: category, familycode: family, subfamilycode: subfamily, Method: "BindSubSubFamily" },
        success: function (data) {
            // 
