...
from
  (
   ...
   select * from WF_APPROVALHISTORY as a
    ...
    WHERE
      T.PROCESSNAME = CL.EXT01
      AND T.INCIDENT = CL.EXT02
      AND CL.SYNCSTATUS = 0
  ) a");
...
on T.FORMID = WF.FORMID AND T.INCIDENT != '-1')");
