...
//using Ultimus.UWF.Form.WebControls;
...
namespace PR.PRProcess.CPR_FOOD
{
    public partial class NewRequest : System.Web.UI.Page
    {
        ...
        {
            //RadioButtonList_Batch.Attributes.Add("onchange ", "SetVisible() ");
            ...
            if (procType.ToUpper().Trim() == "NEWREQUEST")
            {
                ...
                //string manage = Request.QueryString["cprType"];



                CprType = Request.QueryString["CprType"];
                CprArticle = Request.QueryString["ArticleList"];
                SiteCode = Request.QueryString["pccode"];
                if (!string.IsNullOrWhiteSpace(CprArticle))
                {
                    ...
                    //fld_APPLYPURPOSE.SelectedIndex(2);
                }
                ...
                //TextBox fld_APPLICANT = (TextBox)UserInfo.FindControl("fld_APPLICANT");
                //fld_APPLICANT
            ...
            if (procType.ToUpper().Trim() == "REPORT")
            {
                ...
                string url = string.Format("{4}/Solution/PR.PRProcess.CPR_FOOD/Form/NewRequest.aspx?ProcessName=CPR_FOOD&StepName=Begin&Incident={0}&TaskID={1}&UserName={2}&Type=MYREQUEST&ServerName=&t=&FORMID={3}&hasformid=0&t=&processStatus=1&ShowType=REPORT", Incident, TaskID, UserName, FORMID, rootPath);
        ...
        private void BindDropdownlistByLanguage(string userName)
        {
            ...
            if (language == "en-US")
            {
                ...
                RadioButtonList fld_ONLINEORSUPERMARKET = (RadioButtonList)Page.FindControl("fld_ONLINEORSUPERMARKET");
                RadioButtonList fld_FIXEDASSETS = (RadioButtonList)Page.FindControl("fld_FIXEDASSETS");
                ...
                string onlineorsupermarket = fld_ONLINEORSUPERMARKET.SelectedItem.Text;
                ...
                if (onlineorsupermarket == "否")
                {
                    //网上或超市采购
                    BindOnlineOrSupermarket();
                }
                if (fixedassets == "否")
                {
                    //是否是固定资产
        ...
        //private string GetLanguage(string username, string empno)
        //{
        //    string loginName = "";
        //    StringBuilder sSql = new StringBuilder();
        //    if (username != "")
        ...
        //    else if (empno != "")
        //    {
        //        sSql.Append("SELECT LANGUAGE FROM ORG_USER WHERE EMPNO = '" + empno + "'");
        //    }
        ...
        private string GetLanguage(string username)
        {
            ...
            {
                LogUtil.Info("GetLanguage:" + username + "11111");
                loginName = username.Trim().Replace('/', '\\').Split('\\')[1];
                sSql.Append("SELECT LANGUAGE FROM ORG_USER WHERE LOGINNAME='" + loginName + "'");
        ...
        /// 网上或超市采购
        /// </summary>
        private void BindOnlineOrSupermarket()
        {
            RadioButtonList fld_ONLINEORSUPERMARKET = (RadioButtonList)Page.FindControl("fld_ONLINEORSUPERMARKET");
            fld_ONLINEORSUPERMARKET.Items.Clear();
            fld_ONLINEORSUPERMARKET.Items.Add(new ListItem("Yes", "1"));
            fld_ONLINEORSUPERMARKET.Items.Add(new ListItem("No", "0"));
            fld_ONLINEORSUPERMARKET.Items[1].Selected = true;
        }
        /// <summary>
        /// 是否预付
        /// </summary>
        ...
        protected void NewRequest_BeforeSubmit(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (applypurpose_value == "2") //onlineorsupermarket_value == "1" ||
                    {
                        UserInfo UserInfo1 = Page.FindControl("UserInfo1") as UserInfo;
                    ...
                    if (fld_SUPPLIERTYPE.SelectedItem.Value != "0" || fld_SUPPLIERTYPE.SelectedItem.Text != "授权供应商" || fld_SUPPLIERTYPE.SelectedItem.Text != "Authorized Supplier")
                    {
                        AddArticleRepeater();
                    }
        ...
        protected void NewRequest_AfterSubmit(object sender, System.ComponentModel.CancelEventArgs e)
        {
            try
            {
                ...
                if (fld_SUPPLIERTYPE.SelectedItem.Value == "9" || fld_SUPPLIERTYPE.SelectedItem.Text == "授权供应商" || fld_SUPPLIERTYPE.SelectedItem.Text == "Authorized Supplier")
                {
                    ...
                    DataTable Article = _workflow.GetDetailData(UserInfo, fld_detail_PROC_CPRFOOD_Items, UserInfo.FormID);
                    foreach (DataRow item in Article.Rows)
                    {
                        ...
                        LogUtil.Info("NewRequest_AfterSubmit" + sSql.ToString());
                        ...
                        if (ar.Rows.Count > 0)
                        {
                            ...
                            if (IsOneTimeUsing == "1")
                            {
                                ...
                                LogUtil.Info("UseTimes" + sSql.ToString());
        ...
        private void AddArticleRepeater()
        {
            ...
            foreach (DataRow item in dt2.Rows)
            {
                ...
                if (!boolArticle)
                {
                    TextBox fld_ISHISTORY = (TextBox)Page.FindControl("fld_ISHISTORY");
                    fld_ISHISTORY.Text = "0";
...
@UOM_UseUnit,@Gross_weight,@NetVolume,@NetVolumeUnit,@CreateDesc,@CreateDate)");
        ...
        {
            string DateNow = DateTime.Now.ToString("yyyy-MM-dd");
            string sql = string.Format("SELECT DISTINCT TOP 5000 [ArticleCode]  FROM [dbo].[SODEXO_Article]  join [dbo].SODEXO_t_Foun_SCM_FamilyHierarchy fh on fh.CategoryNameEN='Food' and ArticleFamily=fh.SubSubFamilyCode and EffictiveEndDate>='{0}' and SiteCode='{1}'  and SitePrice>0  and  ((IsOneTimeUsing=1 AND UseTimes=0)) AND RFQ_Number IS NOT NULL", DateNow, SiteCode);
        ...
        //    string lan = GetLanguage(user_name);
        //    if (lan.ToLower() == "en-us")
        //    {
        //        if (USER_SIGNEDAPPROVER != "")
        //        {
        //            string domain = "CustomOC";
        //            string loginName1 = USER_SIGNEDAPPROVER;
        //            fld_USER_SIGNEDAPPROVER.Text = "USER:org=" + domain + ",user=" + string.Format("{0}/{1}", domain, loginName1);
        //            if (USER_SIGNEDAPPROVER2 != "")
        //            {
        //                string loginName2 = USER_SIGNEDAPPROVER2;
        //                fld_USER_SIGNEDAPPROVER2.Text = "USER:org=" + domain + ",user=" + string.Format("{0}/{1}", domain, loginName2);
        //                if (USER_SIGNEDAPPROVER3 != "")
        //                {
        //                    string loginName3 = USER_SIGNEDAPPROVER3;
        //                    fld_USER_SIGNEDAPPROVER3.Text = "USER:org=" + domain + ",user=" + string.Format("{0}/{1}", domain, loginName3);
        //                    fld_SIGNEDAPPROVERNUMBER.Text = "3";
        //                }
        //                else
        //                {
        ...
        //            else
        //            {
        //                string domain1 = USER_SIGNEDAPPROVER.Split('\\')[0];
        //                string loginName1 = USER_SIGNEDAPPROVER.Split('\\')[1];
        //                fld_USER_SIGNEDAPPROVER.Text = "USER:org=" + domain1 + ",user=" + string.Format("{0}/{1}", domain1, loginName1);
        //                if (USER_SIGNEDAPPROVER2 != "")
        //                {
        //                    string domain2 = USER_SIGNEDAPPROVER2.Split('\\')[0];
        //                    string loginName2 = USER_SIGNEDAPPROVER2.Split('\\')[1];
        //                    fld_USER_SIGNEDAPPROVER2.Text = "USER:org=" + domain2 + ",user=" + string.Format("{0}/{1}", domain2, loginName2);
        //                    if (USER_SIGNEDAPPROVER3 != "")
        //                    {
        //                        string domain3 = USER_SIGNEDAPPROVER3.Split('\\')[0];
        //                        string loginName3 = USER_SIGNEDAPPROVER3.Split('\\')[1];
        //                        fld_USER_SIGNEDAPPROVER3.Text = "USER:org=" + domain3 + ",user=" + string.Format("{0}/{1}", domain3, loginName3);
        //                        fld_SIGNEDAPPROVERNUMBER.Text = "3";
        //                    }
        //                    else
        //                    {
        ...
        private string CheckOrderQuantityNum()
        {
            string ArticleName = null;
            UserInfo UserInfo = Page.FindControl("UserInfo1") as UserInfo;
            Ultimus.UWF.Form.WebControls.Repeater fld_detail_PROC_CPRFOOD_Items = Page.FindControl("fld_detail_PROC_CPRFOOD_Items") as Ultimus.UWF.Form.WebControls.Repeater;
            ...
            {
                string OrderQuantity = item["OrderQuantity"].ToString();
                string netvomuleunit = item["ORDERUNIT"].ToString();
                if (OrderQuantity == "0")
                {
                    ArticleName += string.Format("{0},", item["ARTICLENAME"].ToString());
                }
                else if (netvomuleunit == "KG" || netvomuleunit == "Hour" || netvomuleunit == "Meter" || netvomuleunit == "Square meter" || netvomuleunit == "小时" || netvomuleunit == "米" || netvomuleunit == "平方" || netvomuleunit == "千克" || netvomuleunit == "克" || netvomuleunit == "G"  || netvomuleunit == "M"||netvomuleunit == "M2" || netvomuleunit == "M3" || netvomuleunit == "平方米")
                {
                    int num = OrderQuantity.Split('.')[1].Length;
                    if (num > 1)
                    {
                        ArticleName += string.Format("{0},", item["ARTICLENAME"].ToString());
                    }
