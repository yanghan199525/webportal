...
var searchSupplier = "";
var searchArticles = "";
var ddlTaxnomyFive = $('#ddlTaxnomyFive');
var ddlTaxnomySix = $('#ddlTaxnomySix');
...
$(function () {
    ...
    if (language.toLowerCase() == 'en-us') {
        ...
        ddlTaxnomyFive.selectpicker({
            noneSelectedText: 'Please Select',
        });
        ddlTaxnomySix.selectpicker({
            noneSelectedText: 'Please Select',
        });
    }
    else {
    ...
    //});
    var level = 2;
    BindFamily(category, level, "", ddlFamily);
    ddlFamily.on('change', function (e) {
        // 
        if (suppliertype != '9') {
            BindFamily(category, level + 1, ddlFamily.val(), ddlSubFamily);
        }
        $("#hiddenTAXNOMYLASTCODE").val(ddlFamily.val());
        refreshTaxnomy(level);
    ...
    ddlSubFamily.on('change', function (e) {
        ...
        debugger
        if (suppliertype != '9') {
            BindFamily(category, level + 2, ddlSubFamily.val(), ddlSubSubFamily);
            // BindSubSubFamily(category, ddlFamily.val(), ddlSubFamily.val());
        }
        $("#hiddenTAXNOMYLASTCODE").val(ddlSubFamily.val());
        refreshTaxnomy(level + 1);
    ...
    ddlSubSubFamily.on('change', function (e) {
        ...
        debugger
        if (suppliertype != '9') {
            BindFamily(category, level + 3, ddlSubSubFamily.val(), ddlTaxnomyFive);
            // BindSubSubFamily(category, ddlFamily.val(), ddlSubFamily.val());
        }
        $("#hiddenTAXNOMYLASTCODE").val(ddlSubSubFamily.val());
        refreshTaxnomy(level + 2);

    });
    ddlTaxnomyFive.on('change', function (e) {
        // 
        debugger
        if (suppliertype != '9') {
            BindFamily(category, level + 4, ddlTaxnomyFive.val(), ddlTaxnomySix);
            // BindSubSubFamily(category, ddlFamily.val(), ddlSubFamily.val());
        }
        $("#hiddenTAXNOMYLASTCODE").val(ddlTaxnomyFive.val());
        refreshTaxnomy(level + 3);
    ...
    ddlTaxnomySix.on('change', function (e) {
        debugger
        if ($('#ddlSubSubFamily').val() != "") {
            BindSubSubFamilyCE();
        ...
        $("#hiddenTAXNOMYLASTCODE").val(ddlTaxnomySix.val());
    });
        ...
        //}
            ...
            if (OTHERARTICLENAME == '') {
                if (ArticleText == '' || ArticleText == '请选择' || ArticleText == 'Please Select') {
                    ...
                    rowdata += "\"subsubfamilycode\":\"" + $('#ddlSubSubFamily').val() + "\",";
                    rowdata += "\"subsubfamilyce\":\"" + $("#hdSubSubFamilyCe").val() + "\",";
                    rowdata += "\"taxnomyfive\":\"" + $('#ddlTaxnomyFive').find("option:selected").text() + "\",";
                    rowdata += "\"taxnomyfivecode\":\"" + $('#ddlTaxnomyFive').val() + "\",";

                    rowdata += "\"taxnomysix\":\"" + $('#ddlTaxnomySix').find("option:selected").text() + "\",";
                    rowdata += "\"taxnomysixcode\":\"" + $('#ddlTaxnomySix').val() + "\",";

                    rowdata += "\"taxnomylastcode\":\"" + $('#hiddenTAXNOMYLASTCODE').val() + "\",";
...
function ClearDialog() {
    ...
    $('#ddlOrderUnit,#ddlUnit,#ddlConsumptionUnit,#ddlSubFamily,#ddlSubSubFamily,#ddlFamily,#ddlArticle,#ddlArticles,#ddlTaxnomyFive,#ddlTaxnomySix').selectpicker('val', '');
    $('#ddlOrderUnit,#ddlUnit,#ddlConsumptionUnit,#ddlSubFamily,#ddlSubSubFamily,#ddlFamily,#ddlArticle,#ddlArticles,#ddlTaxnomyFive,#ddlTaxnomySix').selectpicker('refresh');
...
function checkPositiveInteger6(field, rules, i, options) {
...
function refreshTaxnomy(level) {
    switch (level) {
        case 2:
            ...
            ddlSubSubFamily.selectpicker('val', '');
            ddlSubSubFamily.selectpicker('refresh');
            ddlTaxnomyFive.selectpicker('val', '');
            ddlTaxnomyFive.selectpicker('refresh');
            ddlTaxnomySix.selectpicker('val', '');
            ddlTaxnomySix.selectpicker('refresh');
            break;
...
function BindFamily(category, level, familycode, ddlFamily) {
    $.ajax({
        type: "POST",
        datatype: "json",
        contentType: "application/json",
        url: 'AddPRItemPage.aspx/BindTaxonomy',
        data: "{\"categorycode\":\"" + category + "\",\"level\":\"" + level + "\",\"familycode\":\"" + familycode + "\"}",
        ...
        success: function (data) {
            if (data.d != "") {
                ...
                if (language.toLowerCase() == "en-us") {
                    ddlFamily.append("<option value=''>Please Select</option>");
                    for (var i = 0; i < arrData.length; i++) {
                        ddlFamily.append("<option value='" + arrData[i].TaxonomyCode + "'>" + arrData[i].TaxonomyNameEN + "</option>");
                    }
                }
                else {
                    ddlFamily.append("<option value=''>请选择</option>");
                    for (var i = 0; i < arrData.length; i++) {
                        ddlFamily.append("<option value='" + arrData[i].TaxonomyCode + "'>" + arrData[i].TaxonomyNameCN + "</option>");
                    }
                }
                ddlFamily.selectpicker('val', '');
                ddlFamily.selectpicker('refresh');
                ...
                //$('#ddlSubFamily,#ddlSubSubFamily,#ddlTaxnomyFive,#ddlTaxnomySix').empty();
                //$('#ddlSubFamily,#ddlSubSubFamily,#ddlTaxnomyFive,#ddlTaxnomySix').selectpicker('val', '');
                //$('#ddlSubFamily,#ddlSubSubFamily,#ddlTaxnomyFive,#ddlTaxnomySix').selectpicker('refresh');
                //ddlFamily.parent().children('div').find('button').attr("id", "selectbtnfamily");
            }
        }
    });
