    ...
    public partial class NewRequest : System.Web.UI.Page
    {
        ...
        protected void AfterLoad()
        {
            ...
            if (procType.ToUpper().Trim() == "REPORT")
            {
                ...
                string url = string.Format("{4}/Solution/PR.PRProcess.HK_CPR_NONFOOD/Form/NewRequest.aspx?ProcessName=HK_CPR_NONFOOD&StepName=Begin&Incident={0}&TaskID={1}&UserName={2}&Type=MYREQUEST&ServerName=&t=&FORMID={3}&hasformid=0&t=&processStatus=1&ShowType=REPORT", Incident, TaskID, UserName, FORMID, rootPath);
        ...
        protected void NewRequest_AfterSubmit(object sender, System.ComponentModel.CancelEventArgs e)
        {
            try
            {
                ...
                if (fld_SUPPLIERTYPE.SelectedItem.Value == "9" || fld_SUPPLIERTYPE.SelectedItem.Text == "授權供應商" || fld_SUPPLIERTYPE.SelectedItem.Text == "Authorized Supplier")
                {
                    ...
                    DataTable Article = _workflow.GetDetailData(UserInfo, fld_detail_PROC_HK_CPR_NONFOOD_Items, UserInfo.FormID);
        ...
        private void AddArticleRepeater()
        {
            ...
            Ultimus.UWF.Form.WebControls.Repeater fld_detail_PROC_HK_CPR_NONFOOD_Items = Page.FindControl("fld_detail_PROC_HK_CPR_NONFOOD_Items") as Ultimus.UWF.Form.WebControls.Repeater;
            IWorkflow _workflow = ServiceContainer.Instance().GetService<IWorkflow>();
            DataTable dt2 = _workflow.GetDetailData(UserInfo, fld_detail_PROC_HK_CPR_NONFOOD_Items, UserInfo.FormID);
...
@UOM_UseUnit,@Gross_weight,@NetVolume,@NetVolumeUnit,@CreateDesc,@CreateDate)");
        ...
        private string[] checkArticleCode()
        {
            ...
            //            if (i != j)
            //            {
            //                string article_name = Article.Rows[j]["ARTICLENAME"].ToString();
            //                if (articlename == article_name)
            //                {
