...
{
    public class GetSodexoPDFOOD : GetRecipient
    {
        /// <summary>
        /// 获取sodexo食品采购总监
        /// </summary>
...
");
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    {                      
                        ...
                        {
                            loginName = OrgLogic.GetUserLoginNameByEmpNo(dr["APPROVER"].ToString());
                            if (stepConfig.Process == "PO_SUPPLIER_SUMMARY")
                            {
                                string assignedToUser = ConvertUtil.ToString(bussData.Rows[0]["ASSIGNEDTOUSER"]);
                                //var chechDt = DoSpecialProcessed.GetDirector(assignedToUser);
                                //if (chechDt != null && chechDt.Rows.Count > 0)
                                //{
                                //    var rddata = chechDt.AsEnumerable().FirstOrDefault();
                                //    if (rddata != null)
                                //    {
                                //        loginName = rddata["SOURCINGDIRECTOR"].ToString();
                                //    }
                                //}
                                loginName = assignedToUser;
                            }                        
