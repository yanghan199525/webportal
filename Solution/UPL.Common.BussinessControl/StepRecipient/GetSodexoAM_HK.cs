...
{
    public class GetSodexoAM_HK : GetRecipient
    {
        /// <summary>
...
");
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        {
                            var rddata = chechDt.AsEnumerable().Where(r => r["Scope"].ToString() == "AM").Where(x => x["PCCODE"].ToString() == pccode);
                    ...
                    {
                        throw new Exception(string.Format("区域经理获取失败：分店编号{0},请联系管理员！", pccode));
                    }
                    if (string.IsNullOrEmpty(leaderNumber))
                    {
                        throw new Exception(string.Format("区域经理获取失败：分店编号{0},区域经理账号为空，请联系管理员！", pccode));
                    }
                    if (string.IsNullOrEmpty(loginName))
                    {
                        throw new Exception(string.Format("区域经理获取失败：分店编号{0},区域经理登录名为空，请联系管理员！", pccode));
                    }
