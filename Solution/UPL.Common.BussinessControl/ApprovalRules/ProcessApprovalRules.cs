...
namespace UPL.Common.BussinessControl.ApprovalRules
{
    ...
    /// 所有流程审批规则
    /// </summary>
    public class ProcessApprovalRules
    {
        /// <summary>
        /// 提交表单
        /// </summary>
        ...
        public bool ApprovalRules(DataSet formData, ref Hashtable vars, ref Hashtable users, string loginname, string applicant, string taskID, string processName,
            int incident, string stepName, string tableName, string formID, string prefix, bool isCreateForm, string type, string summary,
            int actionType, string returnStep, string comments, ref string error, string documentNo)
        {
            bool isResult = false;
            try
            {
                ...
                {
                    case "自定义流程":
                        CustomProcessRules Custom = new CustomProcessRules();
                        Custom.CustomProcessA(isCreateForm, formID, stepName, ref vars);
                        break;
                    default:
                        break;
                }
