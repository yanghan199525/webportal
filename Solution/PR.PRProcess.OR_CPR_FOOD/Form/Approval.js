...
function beforeSubmit() {
    ...
    //if ($("#fileinfo").find("tr").length <= 0 && StepName =='SignedApprover') {
    //    alert("请至少上传一个附件");
    //    return false;
    //}
    return true;
...
$(function () {
    ...
    var userName = getUrlParam('UserName').split('\\')[1];
    var LoginName = $("#CheckUser").val().split('\\')[1];
    var Type = getUrlParam('Type').toUpperCase();
    if (StepName == 'Applicant Confirmation') {
        if (Type == 'MYTASK') {
    ...
    if (StepName == "SignedApprover" && userName.toLocaleLowerCase() == LoginName.toLocaleLowerCase()) {
