...
WHERE AAA.SITECODE = '{0}' 
  ...
  AND AAA.INCIDENT <> -1", pcCode, empNo);
            ...
            //cprt.Source = string.Format("BizDB.SELECT top 200000 AAA.[FORMID],AAA.[PROCESSNAME],AAA.[INCIDENT],[DOCUMENTNO],[CREATEBY],[CREATEBYACCOUNT],[CREATEBYCODE],[APPLICANT],[APPLICANTACCOUNT],[APPLICANTCODE],[REQUESTDATE],[COMPLETEDATE],[DEPARTMENT],[DEPARTMENTID],[PROCESSSUMMARY],AAA.[STATUS],[PurchasingPurpose],[SITECODE],[SITENAME],[DELIVERYDATE],[Requirement],[APPROVEDATE],[DELIVERY],[APPROVE],[APPLICANTTEL],[COMPANY],[COSTCENTER],[APPREMARK],AMOUNT,ACTION FROM ( SELECT TOP 200000 * FROM(SELECT [FORMID],[PROCESSNAME],[INCIDENT],[DOCUMENTNO],[CREATEBY],[CREATEBYACCOUNT],[CREATEBYCODE],[APPLICANT],[APPLICANTACCOUNT],[APPLICANTCODE],[REQUESTDATE],[COMPLETEDATE],[DEPARTMENT],[DEPARTMENTID],[PROCESSSUMMARY],[STATUS],[PurchasingPurpose],[SITECODE],[SITENAME],[DELIVERYDATE],[Requirement],[APPROVEDATE],[DELIVERY],[APPROVE],[APPLICANTTEL],[COMPANY],[COSTCENTER],[APPREMARK],AMOUNT FROM PROC_PO_AMENDMENT )T WHERE 1 = 1 AND T.SITECODE = '{0}' AND T.APPLICANTCODE='{1}' ORDER BY T.REQUESTDATE DESC) AAA,(SELECT * FROM WF_APPROVALHISTORY WHERE ID IN(SELECT TOP 200000 MAX(ID) ID FROM WF_APPROVALHISTORY GROUP BY FORMID)) WF WHERE AAA.FORMID=WF.FORMID AND AAA.INCIDENT!='-1' ORDER BY AAA.REQUESTDATE DESC", pcCode, empNo);
