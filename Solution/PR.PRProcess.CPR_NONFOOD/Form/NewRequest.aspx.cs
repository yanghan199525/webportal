    ...
    public partial class NewRequest : System.Web.UI.Page
    {
        ...
        protected void AfterLoad()
        {
            ...
            if (procType.ToUpper().Trim() == "REPORT")
            {
                ...
                string url = string.Format("{4}/Solution/PR.PRProcess.CPR_NONFOOD/Form/NewRequest.aspx?ProcessName=CPR_NONFOOD&StepName=Begin&Incident={0}&TaskID={1}&UserName={2}&Type=MYREQUEST&ServerName=&t=&FORMID={3}&hasformid=0&t=&processStatus=1&ShowType=REPORT", Incident, TaskID, UserName, FORMID, rootPath);
            ...
            if (CprType == "0")
            {
                ...
                if (list != null)
                {
                    ...
                    foreach (var item in list)
                    {
                        ...
                        sSql.AppendFormat(@"
            SELECT DISTINCT TOP 30 [ArticleCode],RFQ_Number,REPLACE(ArticleName, CHAR(10), '') AS [ArticleName],[FamilyCode],FamilyName,SubFamilyCode,SubFamilyNameCN,SubSubFamilyCode,SubSubFamilyNameCN,UOM_PurUnit,OrderUnitCN,OrderUnitAbbr,BaseUnitAbbr,BaseUnitCN,UOM_Pur2InvRate,UOM_Inv2UseRate,UOM_Pur2InvRate,UOM_UseUnit,UOM_Pur2InvRate,UOM_Inv2UseRate,NetVolume,NetVolumeUnit,Gross_weight,SitePrice,NetNetPrice,OrderLimit,a.SupplierName, a.SupplierCode  FROM [dbo].[SODEXO_OrderUnit] ou,SODEXO_BaseUnit,[dbo].[SODEXO_Article] a,[dbo].SODEXO_t_Foun_SCM_FamilyHierarchy fh where ou.OrderUnitAbbr=a.UOM_PurUnit AND  a.EffictiveEndDate>='{0}'and a.SiteCode='{1}' and fh.CategoryNameEN='Non-Food' and a.ArticleFamily=fh.SubSubFamilyCode  AND a.RFQ_Number='{2}' and a.ArticleCode='{3}' and ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0) ORDER BY [ArticleName]", DateNow, SiteCode, item.RFQ_Number.ToString(), item.ArticleCode.ToString());
        ...
        protected void NewRequest_BeforeSubmit(object sender, System.ComponentModel.CancelEventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    DropDownList fld_APPLYPURPOSE = (DropDownList)Page.FindControl("fld_APPLYPURPOSE");
                    string onlineorsupermarket_value = fld_ONLINEORSUPERMARKET.SelectedItem.Value;
        ...
        protected void NewRequest_AfterSubmit(object sender, System.ComponentModel.CancelEventArgs e)
        {
            try
            {
                ...
                if (fld_SUPPLIERTYPE.SelectedItem.Value == "9" || fld_SUPPLIERTYPE.SelectedItem.Text == "授权供应商" || fld_SUPPLIERTYPE.SelectedItem.Text == "Authorized Supplier")
                {
                    ...
                    DataTable Article = _workflow.GetDetailData(UserInfo, fld_detail_PROC_CPR_NONFOOD_ITEMS, UserInfo.FormID);
        ...
        {
            string DateNow = DateTime.Now.ToString("yyyy-MM-dd");
            string sql = string.Format("SELECT DISTINCT TOP 5000 [ArticleCode]  FROM [dbo].[SODEXO_Article]  join [dbo].SODEXO_t_Foun_SCM_FamilyHierarchy fh on fh.CategoryNameEN='Non-Food' and ArticleFamily=fh.SubSubFamilyCode and EffictiveEndDate>='{0}' and SiteCode='{1}'  and SitePrice>0  and  ((IsOneTimeUsing=1 AND UseTimes=0)) AND RFQ_Number IS NOT NULL", DateNow, SiteCode);
        ...
        private void AddArticleRepeater()
        {
            ...
            Ultimus.UWF.Form.WebControls.Repeater fld_detail_PROC_CPR_NONFOOD_ITEMS = Page.FindControl("fld_detail_PROC_CPR_NONFOOD_ITEMS") as Ultimus.UWF.Form.WebControls.Repeater;
            IWorkflow _workflow = ServiceContainer.Instance().GetService<IWorkflow>();
            DataTable dt2 = _workflow.GetDetailData(UserInfo, fld_detail_PROC_CPR_NONFOOD_ITEMS, UserInfo.FormID);
            ...
            foreach (DataRow item in dt2.Rows)
            {
                ...
                if (!boolArticle)
                {
                    ...
                    sSql.Append(@"INSERT INTO [dbo].[SODEXO_Article]
            ...
            if (fld_SUPPLIERTYPE.SelectedItem.Value == "9" || fld_SUPPLIERTYPE.SelectedItem.Text == "授权供应商" || fld_SUPPLIERTYPE.SelectedItem.Text == "Authorized Supplier")
            {
                ...
                foreach (DataRow item in Article.Rows)
                {
                    ...
                    if (ar.Rows.Count > 0)
                    {
                        string IsOneTimeUsing = ar.Rows[0]["IsOneTimeUsing"].ToString();
                        string UseTimes = ar.Rows[0]["UseTimes"].ToString();
                        //string ar_ArticleCode = ar.Rows[0]["ArticleCode"].ToString();
                        if (IsOneTimeUsing == "1" && UseTimes == "1")
                        {
                            //resArray[i] = "false|" + ArticleName + "";
