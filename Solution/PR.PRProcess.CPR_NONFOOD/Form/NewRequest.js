...
$(function () {
    ...
    //将单价转化成千分位
    $("#fld_detail_PROC_CPR_NONFOOD_ITEMS_ctl00_fld_SITEPRICE").next("span").text
        (thousands(numberval($("#fld_detail_PROC_CPR_NONFOOD_ITEMS_ctl00_fld_SITEPRICE").val())));
...
function isOrderQuantity() {
    ...
    for (var i = 1; i < tabCtl.rows.length; i++) {
        ...
        var result = checkPositiveInteger(orderquantity, existrow, fld_ORDERUNIT);
        if (!result) {
            return false;
        }
...
function showInvoiceInfo() {
    ...
    if ($("#fld_SUPPLIERTYPE").val() == "5") {
        ...
        config.tableCols.forEach(function (colClass) {
            $(`#tb_CPR_NONFOOD_ITEMS thead tr td.${colClass}, #tb_CPR_NONFOOD_ITEMS tbody tr td.${colClass}`).show();
        });
    }
    else {
        // 隐藏div
