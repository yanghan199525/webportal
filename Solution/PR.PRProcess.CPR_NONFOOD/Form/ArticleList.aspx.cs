...
namespace PR.PRProcess.CPR_NONFOOD
{
    public partial class ArticleList : System.Web.UI.Page
    {
        ...
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"
        //     SELECT DISTINCT TOP 5000 [ArticleCode],RFQ_Number,REPLACE(ArticleName, CHAR(10), '') AS [ArticleName],[FamilyCode],FamilyName,SubFamilyCode,SubFamilyNameCN,SubSubFamilyCode,SubSubFamilyNameCN,UOM_PurUnit,OrderUnitCN,OrderUnitAbbr,BaseUnitAbbr,BaseUnitCN,UOM_Pur2InvRate,UOM_Inv2UseRate,UOM_UseUnit,NetVolume,NetVolumeUnit,Gross_weight,SitePrice,NetNetPrice,OrderLimit,a.SupplierName, a.SupplierCode,tax.TaxCode,a.TaxRate,tax.InvoiceTypeDesc  FROM [dbo].[SODEXO_Article] a left join SODEXO_t_Foun_VAT_Config tax on tax.TaxRate=a.TaxRate and tax.InvoiceType=a.InvoiceType left join [SODEXO_OrderUnit] ou on ou.OrderUnitAbbr=a.UOM_PurUnit left join SODEXO_BaseUnit bu on bu.BaseUnitAbbr=a.UOM_UseUnit and a.UOM_InvUnit=bu.BaseUnitAbbr join [dbo].SODEXO_t_Foun_SCM_FamilyHierarchy fh on fh.CategoryNameEN='Non-Food' and a.ArticleFamily=fh.SubSubFamilyCode and a.EffictiveEndDate>={0} and a.SiteCode='{1}' and a.ArticleName like N'%{2}%' and SitePrice>0  and  ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0)", DateNow, siteCode, ArticleName);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        //    else if (RFQ_Number != "" && SupperCode != "" && ArticleName == "" && string.IsNullOrWhiteSpace(familyCode))
        //    {
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"
        //     SELECT DISTINCT TOP 5000 [ArticleCode],RFQ_Number,REPLACE(ArticleName, CHAR(10), '') AS [ArticleName],[FamilyCode],FamilyName,SubFamilyCode,SubFamilyNameCN,SubSubFamilyCode,SubSubFamilyNameCN,UOM_PurUnit,OrderUnitCN,OrderUnitAbbr,BaseUnitAbbr,BaseUnitCN,UOM_Pur2InvRate,UOM_Inv2UseRate,UOM_UseUnit,NetVolume,NetVolumeUnit,Gross_weight,SitePrice,NetNetPrice,OrderLimit,a.SupplierName, a.SupplierCode,tax.TaxCode,a.TaxRate,tax.InvoiceTypeDesc  FROM [dbo].[SODEXO_Article] a left join SODEXO_t_Foun_VAT_Config tax on tax.TaxRate=a.TaxRate and tax.InvoiceType=a.InvoiceType left join [SODEXO_OrderUnit] ou on ou.OrderUnitAbbr=a.UOM_PurUnit left join SODEXO_BaseUnit bu on bu.BaseUnitAbbr=a.UOM_UseUnit and a.UOM_InvUnit=bu.BaseUnitAbbr join [dbo].SODEXO_t_Foun_SCM_FamilyHierarchy fh on fh.CategoryNameEN='Non-Food' and a.ArticleFamily=fh.SubSubFamilyCode and a.EffictiveEndDate>={0} and a.SiteCode='{1}' and RFQ_Number='{2}'  and a.SupplierCode='{3}' and SitePrice>0  and  ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0)", DateNow, siteCode, RFQ_Number, SupperCode);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        //    else if (RFQ_Number != "" && SupperCode == "" && ArticleName != "" && string.IsNullOrWhiteSpace(familyCode))
        //    {
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"
        //      SELECT DISTINCT TOP 5000 [ArticleCode],RFQ_Number,REPLACE(ArticleName, CHAR(10), '') AS [ArticleName],[FamilyCode],FamilyName,SubFamilyCode,SubFamilyNameCN,SubSubFamilyCode,SubSubFamilyNameCN,UOM_PurUnit,OrderUnitCN,OrderUnitAbbr,BaseUnitAbbr,BaseUnitCN,UOM_Pur2InvRate,UOM_Inv2UseRate,UOM_UseUnit,NetVolume,NetVolumeUnit,Gross_weight,SitePrice,NetNetPrice,OrderLimit,a.SupplierName, a.SupplierCode,tax.TaxCode,a.TaxRate,tax.InvoiceTypeDesc  FROM [dbo].[SODEXO_Article] a left join SODEXO_t_Foun_VAT_Config tax on tax.TaxRate=a.TaxRate and tax.InvoiceType=a.InvoiceType left join [SODEXO_OrderUnit] ou on ou.OrderUnitAbbr=a.UOM_PurUnit left join SODEXO_BaseUnit bu on bu.BaseUnitAbbr=a.UOM_UseUnit and a.UOM_InvUnit=bu.BaseUnitAbbr join [dbo].SODEXO_t_Foun_SCM_FamilyHierarchy fh on fh.CategoryNameEN='Non-Food' and a.ArticleFamily=fh.SubSubFamilyCode and a.EffictiveEndDate>={0} and a.SiteCode='{1}' and RFQ_Number='{2}'  and a.ArticleName like N'%{3}%' and SitePrice>0  and  ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0)", DateNow, siteCode, RFQ_Number, ArticleName);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        //    else if (RFQ_Number == "" && SupperCode != "" && ArticleName != "" && string.IsNullOrWhiteSpace(familyCode))
        //    {
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"
        //     SELECT DISTINCT TOP 5000 [ArticleCode],RFQ_Number,REPLACE(ArticleName, CHAR(10), '') AS [ArticleName],[FamilyCode],FamilyName,SubFamilyCode,SubFamilyNameCN,SubSubFamilyCode,SubSubFamilyNameCN,UOM_PurUnit,OrderUnitCN,OrderUnitAbbr,BaseUnitAbbr,BaseUnitCN,UOM_Pur2InvRate,UOM_Inv2UseRate,UOM_UseUnit,NetVolume,NetVolumeUnit,Gross_weight,SitePrice,NetNetPrice,OrderLimit,a.SupplierName, a.SupplierCode,tax.TaxCode,a.TaxRate,tax.InvoiceTypeDesc  FROM [dbo].[SODEXO_Article] a left join SODEXO_t_Foun_VAT_Config tax on tax.TaxRate=a.TaxRate and tax.InvoiceType=a.InvoiceType left join [SODEXO_OrderUnit] ou on ou.OrderUnitAbbr=a.UOM_PurUnit left join SODEXO_BaseUnit bu on bu.BaseUnitAbbr=a.UOM_UseUnit and a.UOM_InvUnit=bu.BaseUnitAbbr join [dbo].SODEXO_t_Foun_SCM_FamilyHierarchy fh on fh.CategoryNameEN='Non-Food' and a.ArticleFamily=fh.SubSubFamilyCode and a.EffictiveEndDate>={0} and a.SiteCode='{1}' and SupplierCode='{2}'  and a.ArticleName like N'%{3}%' and SitePrice>0  and  ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0)", DateNow, siteCode, SupperCode, ArticleName);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        //    else if (RFQ_Number != "" && SupperCode != "" && ArticleName != "" && string.IsNullOrWhiteSpace(familyCode))
        //    {
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"
        //    SELECT DISTINCT TOP 5000 [ArticleCode],RFQ_Number,REPLACE(ArticleName, CHAR(10), '') AS [ArticleName],[FamilyCode],FamilyName,SubFamilyCode,SubFamilyNameCN,SubSubFamilyCode,SubSubFamilyNameCN,UOM_PurUnit,OrderUnitCN,OrderUnitAbbr,BaseUnitAbbr,BaseUnitCN,UOM_Pur2InvRate,UOM_Inv2UseRate,UOM_UseUnit,NetVolume,NetVolumeUnit,Gross_weight,SitePrice,NetNetPrice,OrderLimit,a.SupplierName, a.SupplierCode,tax.TaxCode,a.TaxRate,tax.InvoiceTypeDesc  FROM [dbo].[SODEXO_Article] a left join SODEXO_t_Foun_VAT_Config tax on tax.TaxRate=a.TaxRate and tax.InvoiceType=a.InvoiceType left join [SODEXO_OrderUnit] ou on ou.OrderUnitAbbr=a.UOM_PurUnit left join SODEXO_BaseUnit bu on bu.BaseUnitAbbr=a.UOM_UseUnit and a.UOM_InvUnit=bu.BaseUnitAbbr join [dbo].SODEXO_t_Foun_SCM_FamilyHierarchy fh on fh.CategoryNameEN='Non-Food' and a.ArticleFamily=fh.SubSubFamilyCode and a.EffictiveEndDate>={0} and a.SiteCode='{1}' and RFQ_Number='{2}' and SupplierCode='{3}'  and a.ArticleName like N'%{4}%' and SitePrice>0  and  ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0)", DateNow, siteCode, RFQ_Number, SupperCode, ArticleName);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        //    else if (RFQ_Number != "" && SupperCode != "" && ArticleName == "" && !string.IsNullOrWhiteSpace(familyCode))
        //    {
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"
        //    SELECT DISTINCT TOP 5000 [ArticleCode],RFQ_Number,REPLACE(ArticleName, CHAR(10), '') AS [ArticleName],[FamilyCode],FamilyName,SubFamilyCode,SubFamilyNameCN,SubSubFamilyCode,SubSubFamilyNameCN,UOM_PurUnit,OrderUnitCN,OrderUnitAbbr,BaseUnitAbbr,BaseUnitCN,UOM_Pur2InvRate,UOM_Inv2UseRate,UOM_UseUnit,NetVolume,NetVolumeUnit,Gross_weight,SitePrice,NetNetPrice,OrderLimit,a.SupplierName, a.SupplierCode,tax.TaxCode,a.TaxRate,tax.InvoiceTypeDesc  FROM [dbo].[SODEXO_Article] a left join SODEXO_t_Foun_VAT_Config tax on tax.TaxRate=a.TaxRate and tax.InvoiceType=a.InvoiceType left join [SODEXO_OrderUnit] ou on ou.OrderUnitAbbr=a.UOM_PurUnit left join SODEXO_BaseUnit bu on bu.BaseUnitAbbr=a.UOM_UseUnit and a.UOM_InvUnit=bu.BaseUnitAbbr join [dbo].SODEXO_t_Foun_SCM_FamilyHierarchy fh on fh.CategoryNameEN='Non-Food' and SupplierCode='a.ArticleFamily=fh.SubSubFamilyCode and a.EffictiveEndDate>={0} and a.SiteCode='{1}' and RFQ_Number='{2}' and SupplierCode='{3}'  and fh.familyCode='{4}' and SitePrice>0  and  ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0)", DateNow, siteCode, RFQ_Number, SupperCode, familyCode);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        //    else if (RFQ_Number != "" && SupperCode != "" && ArticleName != "" && !string.IsNullOrWhiteSpace(familyCode))
        //    {
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"
        //    SELECT DISTINCT TOP 5000 [ArticleCode],RFQ_Number,REPLACE(ArticleName, CHAR(10), '') AS [ArticleName],[FamilyCode],FamilyName,SubFamilyCode,SubFamilyNameCN,SubSubFamilyCode,SubSubFamilyNameCN,UOM_PurUnit,OrderUnitCN,OrderUnitAbbr,BaseUnitAbbr,BaseUnitCN,UOM_Pur2InvRate,UOM_Inv2UseRate,UOM_UseUnit,NetVolume,NetVolumeUnit,Gross_weight,SitePrice,NetNetPrice,OrderLimit,a.SupplierName, a.SupplierCode,tax.TaxCode,a.TaxRate,tax.InvoiceTypeDesc  FROM [dbo].[SODEXO_Article] a left join SODEXO_t_Foun_VAT_Config tax on tax.TaxRate=a.TaxRate and tax.InvoiceType=a.InvoiceType left join [SODEXO_OrderUnit] ou on ou.OrderUnitAbbr=a.UOM_PurUnit left join SODEXO_BaseUnit bu on bu.BaseUnitAbbr=a.UOM_UseUnit and a.UOM_InvUnit=bu.BaseUnitAbbr join [dbo].SODEXO_t_Foun_SCM_FamilyHierarchy fh on fh.CategoryNameEN='Non-Food' and a.ArticleFamily=fh.SubSubFamilyCode and a.EffictiveEndDate>={0} and a.SiteCode='{1}' and RFQ_Number='{2}' and SupplierCode='{3}' and a.ArticleName like N'%{4}%'   and fh.familyCode='{5}' and SitePrice>0  and  ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0)", DateNow, siteCode, RFQ_Number, SupperCode, ArticleName, familyCode);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        //    else if (RFQ_Number == "" && SupperCode == "" && ArticleName == "" && !string.IsNullOrWhiteSpace(familyCode))
        //    {
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"
        //     SELECT DISTINCT TOP 5000 [ArticleCode],RFQ_Number,REPLACE(ArticleName, CHAR(10), '') AS [ArticleName],[FamilyCode],FamilyName,SubFamilyCode,SubFamilyNameCN,SubSubFamilyCode,SubSubFamilyNameCN,UOM_PurUnit,OrderUnitCN,OrderUnitAbbr,BaseUnitAbbr,BaseUnitCN,UOM_Pur2InvRate,UOM_Inv2UseRate,UOM_UseUnit,NetVolume,NetVolumeUnit,Gross_weight,SitePrice,NetNetPrice,OrderLimit,a.SupplierName, a.SupplierCode,tax.TaxCode,a.TaxRate,tax.InvoiceTypeDesc  FROM [dbo].[SODEXO_Article] a left join SODEXO_t_Foun_VAT_Config tax on tax.TaxRate=a.TaxRate and tax.InvoiceType=a.InvoiceType left join [SODEXO_OrderUnit] ou on ou.OrderUnitAbbr=a.UOM_PurUnit left join SODEXO_BaseUnit bu on bu.BaseUnitAbbr=a.UOM_UseUnit and a.UOM_InvUnit=bu.BaseUnitAbbr join [dbo].SODEXO_t_Foun_SCM_FamilyHierarchy fh on fh.CategoryNameEN='Non-Food' and a.ArticleFamily=fh.SubSubFamilyCode and a.EffictiveEndDate>={0} and a.SiteCode='{1}' and fh.familyCode='{2}' and SitePrice>0  and  ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0)", DateNow, siteCode, familyCode);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        //    else if (RFQ_Number != "" && SupperCode == "" && ArticleName == "" && !string.IsNullOrWhiteSpace(familyCode))
        //    {
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"
        //     SELECT DISTINCT TOP 5000 [ArticleCode],RFQ_Number,REPLACE(ArticleName, CHAR(10), '') AS [ArticleName],[FamilyCode],FamilyName,SubFamilyCode,SubFamilyNameCN,SubSubFamilyCode,SubSubFamilyNameCN,UOM_PurUnit,OrderUnitCN,OrderUnitAbbr,BaseUnitAbbr,BaseUnitCN,UOM_Pur2InvRate,UOM_Inv2UseRate,UOM_UseUnit,NetVolume,NetVolumeUnit,Gross_weight,SitePrice,NetNetPrice,OrderLimit,a.SupplierName, a.SupplierCode,tax.TaxCode,a.TaxRate,tax.InvoiceTypeDesc  FROM [dbo].[SODEXO_Article] a left join SODEXO_t_Foun_VAT_Config tax on tax.TaxRate=a.TaxRate and tax.InvoiceType=a.InvoiceType left join [SODEXO_OrderUnit] ou on ou.OrderUnitAbbr=a.UOM_PurUnit left join SODEXO_BaseUnit bu on bu.BaseUnitAbbr=a.UOM_UseUnit and a.UOM_InvUnit=bu.BaseUnitAbbr join [dbo].SODEXO_t_Foun_SCM_FamilyHierarchy fh on fh.CategoryNameEN='Non-Food' and a.ArticleFamily=fh.SubSubFamilyCode and a.EffictiveEndDate>={0} and a.SiteCode='{1}' and RFQ_Number='{2}' and  fh.familyCode='{3}' and SitePrice>0  and  ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0)", DateNow, siteCode, RFQ_Number, familyCode);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        //    else if (RFQ_Number == "" && SupperCode != "" && ArticleName == "" && !string.IsNullOrWhiteSpace(familyCode))
        //    {
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"
        //     SELECT DISTINCT TOP 5000 [ArticleCode],RFQ_Number,REPLACE(ArticleName, CHAR(10), '') AS [ArticleName],[FamilyCode],FamilyName,SubFamilyCode,SubFamilyNameCN,SubSubFamilyCode,SubSubFamilyNameCN,UOM_PurUnit,OrderUnitCN,OrderUnitAbbr,BaseUnitAbbr,BaseUnitCN,UOM_Pur2InvRate,UOM_Inv2UseRate,UOM_UseUnit,NetVolume,NetVolumeUnit,Gross_weight,SitePrice,NetNetPrice,OrderLimit,a.SupplierName, a.SupplierCode,tax.TaxCode,a.TaxRate,tax.InvoiceTypeDesc  FROM [dbo].[SODEXO_Article] a left join SODEXO_t_Foun_VAT_Config tax on tax.TaxRate=a.TaxRate and tax.InvoiceType=a.InvoiceType left join [SODEXO_OrderUnit] ou on ou.OrderUnitAbbr=a.UOM_PurUnit left join SODEXO_BaseUnit bu on bu.BaseUnitAbbr=a.UOM_UseUnit and a.UOM_InvUnit=bu.BaseUnitAbbr join [dbo].SODEXO_t_Foun_SCM_FamilyHierarchy fh on fh.CategoryNameEN='Non-Food' and a.ArticleFamily=fh.SubSubFamilyCode and a.EffictiveEndDate>={0} and a.SiteCode='{1}' and SupplierCode='{2}' and  fh.familyCode='{3}' and SitePrice>0  and  ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0)", DateNow, siteCode, SupperCode, familyCode);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        //    else if (RFQ_Number == "" && SupperCode == "" && ArticleName != "" && !string.IsNullOrWhiteSpace(familyCode))
        //    {
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"
        //     SELECT DISTINCT TOP 5000 [ArticleCode],RFQ_Number,REPLACE(ArticleName, CHAR(10), '') AS [ArticleName],[FamilyCode],FamilyName,SubFamilyCode,SubFamilyNameCN,SubSubFamilyCode,SubSubFamilyNameCN,UOM_PurUnit,OrderUnitCN,OrderUnitAbbr,BaseUnitAbbr,BaseUnitCN,UOM_Pur2InvRate,UOM_Inv2UseRate,UOM_UseUnit,NetVolume,NetVolumeUnit,Gross_weight,SitePrice,NetNetPrice,OrderLimit,a.SupplierName, a.SupplierCode,tax.TaxCode,a.TaxRate,tax.InvoiceTypeDesc  FROM [dbo].[SODEXO_Article] a left join SODEXO_t_Foun_VAT_Config tax on tax.TaxRate=a.TaxRate and tax.InvoiceType=a.InvoiceType left join [SODEXO_OrderUnit] ou on ou.OrderUnitAbbr=a.UOM_PurUnit left join SODEXO_BaseUnit bu on bu.BaseUnitAbbr=a.UOM_UseUnit and a.UOM_InvUnit=bu.BaseUnitAbbr join [dbo].SODEXO_t_Foun_SCM_FamilyHierarchy fh on fh.CategoryNameEN='Non-Food' and a.ArticleFamily=fh.SubSubFamilyCode and a.EffictiveEndDate>={0} and a.SiteCode='{1}' and ArticleName like N'%{2}%' and  fh.familyCode='{3}' and SitePrice>0  and  ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0)", DateNow, siteCode, ArticleName, familyCode);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        //    else if (RFQ_Number != "" && SupperCode != "" && ArticleName == "" && !string.IsNullOrWhiteSpace(familyCode))
        //    {
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"
        //       SELECT DISTINCT TOP 5000 [ArticleCode],RFQ_Number,REPLACE(ArticleName, CHAR(10), '') AS [ArticleName],[FamilyCode],FamilyName,SubFamilyCode,SubFamilyNameCN,SubSubFamilyCode,SubSubFamilyNameCN,UOM_PurUnit,OrderUnitCN,OrderUnitAbbr,BaseUnitAbbr,BaseUnitCN,UOM_Pur2InvRate,UOM_Inv2UseRate,UOM_UseUnit,NetVolume,NetVolumeUnit,Gross_weight,SitePrice,NetNetPrice,OrderLimit,a.SupplierName, a.SupplierCode,tax.TaxCode,a.TaxRate,tax.InvoiceTypeDesc  FROM [dbo].[SODEXO_Article] a left join SODEXO_t_Foun_VAT_Config tax on tax.TaxRate=a.TaxRate and tax.InvoiceType=a.InvoiceType left join [SODEXO_OrderUnit] ou on ou.OrderUnitAbbr=a.UOM_PurUnit left join SODEXO_BaseUnit bu on bu.BaseUnitAbbr=a.UOM_UseUnit and a.UOM_InvUnit=bu.BaseUnitAbbr join [dbo].SODEXO_t_Foun_SCM_FamilyHierarchy fh on fh.CategoryNameEN='Non-Food' and a.ArticleFamily=fh.SubSubFamilyCode and a.EffictiveEndDate>={0} and a.SiteCode='{1}' and RFQ_Number ='{2}' and  fh.familyCode='{3}' and SupplierCode='{4}'  and SitePrice>0  and  ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0)", DateNow, siteCode, RFQ_Number, familyCode, SupperCode);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        //    else if (RFQ_Number != "" && SupperCode == "" && ArticleName != "" && !string.IsNullOrWhiteSpace(familyCode))
        //    {
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"
        //      SELECT DISTINCT TOP 5000 [ArticleCode],RFQ_Number,REPLACE(ArticleName, CHAR(10), '') AS [ArticleName],[FamilyCode],FamilyName,SubFamilyCode,SubFamilyNameCN,SubSubFamilyCode,SubSubFamilyNameCN,UOM_PurUnit,OrderUnitCN,OrderUnitAbbr,BaseUnitAbbr,BaseUnitCN,UOM_Pur2InvRate,UOM_Inv2UseRate,UOM_UseUnit,NetVolume,NetVolumeUnit,Gross_weight,SitePrice,NetNetPrice,OrderLimit,a.SupplierName, a.SupplierCode,tax.TaxCode,a.TaxRate,tax.InvoiceTypeDesc  FROM [dbo].[SODEXO_Article] a left join SODEXO_t_Foun_VAT_Config tax on tax.TaxRate=a.TaxRate and tax.InvoiceType=a.InvoiceType left join [SODEXO_OrderUnit] ou on ou.OrderUnitAbbr=a.UOM_PurUnit left join SODEXO_BaseUnit bu on bu.BaseUnitAbbr=a.UOM_UseUnit and a.UOM_InvUnit=bu.BaseUnitAbbr join [dbo].SODEXO_t_Foun_SCM_FamilyHierarchy fh on fh.CategoryNameEN='Non-Food' and a.ArticleFamily=fh.SubSubFamilyCode and a.EffictiveEndDate>={0} and a.SiteCode='{1}' and RFQ_Number ='{2}' and  fh.familyCode='{3}' and ArticleName like N'%{4}%'  and SitePrice>0  and  ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0)", DateNow, siteCode, RFQ_Number, familyCode, ArticleName);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        //    else if (RFQ_Number == "" && SupperCode != "" && ArticleName != "" && !string.IsNullOrWhiteSpace(familyCode))
        //    {
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"
        //      SELECT DISTINCT TOP 5000 [ArticleCode],RFQ_Number,REPLACE(ArticleName, CHAR(10), '') AS [ArticleName],[FamilyCode],FamilyName,SubFamilyCode,SubFamilyNameCN,SubSubFamilyCode,SubSubFamilyNameCN,UOM_PurUnit,OrderUnitCN,OrderUnitAbbr,BaseUnitAbbr,BaseUnitCN,UOM_Pur2InvRate,UOM_Inv2UseRate,UOM_UseUnit,NetVolume,NetVolumeUnit,Gross_weight,SitePrice,NetNetPrice,OrderLimit,a.SupplierName, a.SupplierCode,tax.TaxCode,a.TaxRate,tax.InvoiceTypeDesc  FROM [dbo].[SODEXO_Article] a left join SODEXO_t_Foun_VAT_Config tax on tax.TaxRate=a.TaxRate and tax.InvoiceType=a.InvoiceType left join [SODEXO_OrderUnit] ou on ou.OrderUnitAbbr=a.UOM_PurUnit left join SODEXO_BaseUnit bu on bu.BaseUnitAbbr=a.UOM_UseUnit and a.UOM_InvUnit=bu.BaseUnitAbbr join [dbo].SODEXO_t_Foun_SCM_FamilyHierarchy fh on fh.CategoryNameEN='Non-Food' and a.ArticleFamily=fh.SubSubFamilyCode and a.EffictiveEndDate>={0} and a.SiteCode='{1}' and SupplierCode ='{2}' and  fh.familyCode='{3}' and ArticleName like N'%{4}%'  and SitePrice>0  and  ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0)", DateNow, siteCode, SupperCode, familyCode, ArticleName);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        //    else
        //    {
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"
        //    SELECT DISTINCT TOP 5000 [ArticleCode],RFQ_Number,REPLACE(ArticleName, CHAR(10), '') AS [ArticleName],[FamilyCode],FamilyName,SubFamilyCode,SubFamilyNameCN,SubSubFamilyCode,SubSubFamilyNameCN,UOM_PurUnit,OrderUnitCN,OrderUnitAbbr,BaseUnitAbbr,BaseUnitCN,UOM_Pur2InvRate,UOM_Inv2UseRate,UOM_UseUnit,NetVolume,NetVolumeUnit,Gross_weight,SitePrice,NetNetPrice,OrderLimit,a.SupplierName, a.SupplierCode,tax.TaxCode,a.TaxRate,tax.InvoiceTypeDesc  FROM [dbo].[SODEXO_Article] a left join SODEXO_t_Foun_VAT_Config tax on tax.TaxRate=a.TaxRate and tax.InvoiceType=a.InvoiceType left join [SODEXO_OrderUnit] ou on ou.OrderUnitAbbr=a.UOM_PurUnit left join SODEXO_BaseUnit bu on bu.BaseUnitAbbr=a.UOM_UseUnit and a.UOM_InvUnit=bu.BaseUnitAbbr join [dbo].SODEXO_t_Foun_SCM_FamilyHierarchy fh on fh.CategoryNameEN='Non-Food' and a.ArticleFamily=fh.SubSubFamilyCode and a.EffictiveEndDate>={0} and a.SiteCode='{1}'  and SitePrice>0  and  ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0)", DateNow, siteCode);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        //    dt = CheckArticle(dt);
        //    return dt;
        ...
        //    {
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"select DISTINCT top 20  b.RFQ_Number from SODEXO_t_Foun_SCM_FamilyHierarchy a,SODEXO_Article  b where a.SubSubFamilyCode=b.ArticleFamily and EffictiveEndDate>='{0}'AND ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0) and a.CategoryNameEN='Non-Food' and SupplierCode like '%{1}%' and SiteCode='{2}'  ", DateNow, searchSupper, siteCode);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        ...
        //    {
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"select DISTINCT top 20  b.RFQ_Number from SODEXO_t_Foun_SCM_FamilyHierarchy a,SODEXO_Article  b where a.SubSubFamilyCode=b.ArticleFamily and EffictiveEndDate>='{0}'AND ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0)  and a.CategoryNameEN='Non-Food' and b.ArticleName like N'%{1}%' and SiteCode='{2}' ", DateNow, searchArticleName, siteCode);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        //    else if ((!string.IsNullOrEmpty(searchArticleName) || searchArticleName != "") && (!string.IsNullOrEmpty(searchSupper) || searchSupper != "") && (string.IsNullOrEmpty(searchRFQInput) || searchRFQInput == ""))
        //    {
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"select DISTINCT top 20  b.RFQ_Number from SODEXO_t_Foun_SCM_FamilyHierarchy a,SODEXO_Article  b where a.SubSubFamilyCode=b.ArticleFamily and EffictiveEndDate>='{0}'AND ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0)  and a.CategoryNameEN='Non-Food' and b.ArticleName like N'%{1}%' and SiteCode='{2}' and b.SupplierCode like '%{3}%' ", DateNow, searchArticleName, siteCode, searchSupper);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        ...
        //    {
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"select DISTINCT top 20  b.RFQ_Number from SODEXO_t_Foun_SCM_FamilyHierarchy a,SODEXO_Article  b where a.SubSubFamilyCode=b.ArticleFamily and EffictiveEndDate>='{0}'AND ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0)  and a.CategoryNameEN='Non-Food' and b.ArticleName like N'%{1}%' and SiteCode='{2}' and b.RFQ_Number like '%{3}%' ", DateNow, searchArticleName, siteCode, searchRFQInput);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        //    else if ((!string.IsNullOrEmpty(searchSupper) || searchSupper != "") && (!string.IsNullOrEmpty(searchRFQInput) || searchRFQInput != "") && (string.IsNullOrEmpty(searchArticleName) || searchArticleName == ""))
        //    {
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"select DISTINCT top 20  b.RFQ_Number from SODEXO_t_Foun_SCM_FamilyHierarchy a,SODEXO_Article  b where a.SubSubFamilyCode=b.ArticleFamily and EffictiveEndDate>='{0}'AND ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0) and a.CategoryNameEN='Non-Food' and b.SupplierCode like '%{1}%' and SiteCode='{2}' and b.RFQ_Number like '%{3}%' ", DateNow, searchSupper, siteCode, searchRFQInput);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        //    else if ((!string.IsNullOrEmpty(searchSupper) || searchSupper != "") && (!string.IsNullOrEmpty(searchRFQInput) || searchRFQInput != "") && (!string.IsNullOrEmpty(searchArticleName) || searchArticleName != ""))
        //    {
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"select DISTINCT top 20  b.RFQ_Number from SODEXO_t_Foun_SCM_FamilyHierarchy a,SODEXO_Article  b where a.SubSubFamilyCode=b.ArticleFamily and EffictiveEndDate>='{0}'AND ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0)  and a.CategoryNameEN='Non-Food' and b.SupplierCode like '%{1}%' and SiteCode='{2}' and b.RFQ_Number like '%{3}%' and b.ArticleName like N'%{4}%' ", DateNow, searchSupper, siteCode, searchRFQInput, searchArticleName);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        //    else
        //    {
        //        sSql.Length = 0;
        //        sSql.AppendFormat(@"select DISTINCT top 20  b.RFQ_Number from SODEXO_t_Foun_SCM_FamilyHierarchy a,SODEXO_Article  b where a.SubSubFamilyCode=b.ArticleFamily and EffictiveEndDate>='{0}'AND ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0) and a.CategoryNameEN='Non-Food'  and SiteCode='{1}'and RFQ_Number is not null", DateNow, siteCode);
        //        dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
        //    }
        ...
        public static string BindSupper(string searchRFQInput, string searchSupper, string searchArticleName)
        {
            ...
            {
                sSql.Length = 0;
                sSql.AppendFormat(@"select DISTINCT top 20  b.SupplierCode from SODEXO_t_Foun_SCM_FamilyHierarchy a,SODEXO_Article  b where a.SubSubFamilyCode=b.ArticleFamily and EffictiveEndDate>='{0}'AND ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0) and  and a.CategoryNameEN='Non-Food' and b.ArticleName like N'%{1}%' and SiteCode='{2}' and b.SupplierCode like '%{3}%' ", DateNow, searchArticleName, siteCode, searchSupper);
                dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
            }
            else if ((!string.IsNullOrEmpty(searchArticleName) || searchArticleName != "") && (!string.IsNullOrEmpty(searchRFQInput) || searchRFQInput != "") && (string.IsNullOrEmpty(searchSupper) || searchSupper == ""))
            {
            ...
            {
                sSql.Length = 0;
                sSql.AppendFormat(@"select DISTINCT top 20  b.SupplierCode from SODEXO_t_Foun_SCM_FamilyHierarchy a,SODEXO_Article  b where a.SubSubFamilyCode=b.ArticleFamily and EffictiveEndDate>='{0}'AND ((IsOneTimeUsing=1 AND UseTimes=0) OR IsOneTimeUsing=0)  and a.CategoryNameEN='Non-Food' and SiteCode='{1}' and SupplierCode is not null", DateNow, siteCode);
                dt = DataAccess.Instance("BizDB").ExecuteDataTable(sSql.ToString());
            }
            string jsonStr = new JsonHelper().DataTableToJson(dt);
            return jsonStr;
...
WHERE
    ...
    AND SiteCode = '" + siteCode + "' " + spWhere + " " + articleWhere + "  " + rfqWhere + "  AND EffictiveEndDate >= '" + DateNow + "' ;");
        ...
        public DataTable BindArticle(string siteCode, string RFQ_Number, string SupperCode, string ArticleName,string familyCode)
        {
            ...
            if (!string.IsNullOrWhiteSpace(familyCode.Trim()))
            {
                articleWhere = "AND familyCode = '" + familyCode + "'";
            }
